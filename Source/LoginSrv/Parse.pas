...
begin
  ...
  while (True) do
  begin
    if (GetTickCount - dwTick2C) > 5 * 60 * 1000 then
    begin
      dwTick2C := GetTickCount();
      try
        if FileExists(Config.sFeedIDList) then
        begin
          AccountLoadList.Clear;
          AccountLoadList.LoadFromFile(Config.sFeedIDList);
          if AccountLoadList.Count > 0 then
          begin
            AccountCostList.Clear;
            for I := 0 to AccountLoadList.Count - 1 do
            begin
          ...
          LoadAccountCostList(AccountCostList);
        end;
      except
      ...
      try
        if FileExists(Config.sFeedIPList) then
        begin
          IPaddrLoadList.Clear;
          IPaddrLoadList.LoadFromFile(Config.sFeedIPList);
          if IPaddrLoadList.Count > 0 then
          begin
            IPaddrCostList.Clear;
            for I := 0 to IPaddrLoadList.Count - 1 do
            begin
              s18 := Trim(IPaddrLoadList.Strings[i]);
              s18 := GetValidStr3(s18, s1C, [' ', #9]);
              s18 := GetValidStr3(s18, s24, [' ', #9]);
              s18 := GetValidStr3(s18, s28, [' ', #9]);
              n10 := StrToIntDef(s24, 0);
              n14 := StrToIntDef(s28, 0);
              nC := MakeLong(_MAX(n14, 0), _MAX(n10, 0));
          ...
          LoadIPaddrCostList(IPaddrCostList);
        end;
      except
