...
begin
  ...
  EdTitle.Text := sTitle;
  nSelRouteIdx := -1;
  for I := Low(Config.GateRoute) to High(Config.GateRoute) do
  begin
    if Config.GateRoute[I].sTitle = sTitle then
    begin
      nSelRouteIdx := I;
      break;
    end;
  ...
  while (true) do
  begin
    if Config.GateRoute[nSelRouteIdx].Gate[nGateIdx].sIPaddr <> '' then
    begin
...
begin
  ...
  begin
    ...
    if (sIPaddr = '') or (StrToIntDef(sPort, 0) = 0) then
    begin
      Beep;
      exit;
    end;
  ...
  while (True) do
  begin
    if g_Config.GateRoute[nGateIdx].sTitle = sTitle then
    begin
      nRouteIdx := nGateIdx;
      break;
    end;
  ...
  g_Config.GateRoute[nRouteIdx].sRemoteAddr := EdPrivateAddr.Text;
  g_Config.GateRoute[nRouteIdx].sPublicAddr := EdPublicAddr.Text;
  nGateIdx := 0;
  while (True) do
  begin
    ...
    if sIPaddr <> '' then
    begin
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].sIPaddr := sIPaddr;
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].nPort := StrToIntDef(sPort, 0);
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].boEnable := CkGate[nGateIdx].Checked;
    end
    else
    begin
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].sIPaddr := '';
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].nPort := 0;
      g_Config.GateRoute[nRouteIdx].Gate[nGateIdx].boEnable := False;
    end;
    Inc(nGateIdx);
    if nGateIdx >= 10 then break;
  end;
...
begin
  ...
  begin
    boAdded := True;
    for II := 0 to CbServerList.Items.Count - 1 do
    begin
