...
type
  ...
  public
    ...
    property IsRun3Grid:Boolean read FIsRun3Grid;
  protected
  ...
  private
    FExcludeMonster:Boolean;
  public
    Title:string;
    FindCount, BeginX, BeginY, EndX, EndY, FindX, FindY, PathPoisonIndex:Integer;
    MapData:TMapInfoArray; //HZQ 20231007 程序中没有用到
    constructor Create;

    procedure Find(StartX, StartY, StopX, StopY:Integer; ExcludeMonster:Boolean = False);
    procedure FindPath(StartX, StartY, StopX, StopY:Integer; PathSpace:Integer = 0; ExcludeMonster:Boolean = False); overload;
    procedure FindPath(StopX, StopY:Integer; ExcludeMonster:Boolean = False); overload;
...
procedure TPathMap.SetWidth(const Value:Integer);
begin
  if FWidth <> Value then begin
    FWidth := Value;
    FRealSize := Bounds(0, 0, FWidth, FHeight);
  end;
end;

procedure TPathMap.SetHeight(const Value:Integer);
begin
...
begin
  ...
  if (FPath <> nil) and (Length(FPath) > 1) then begin
    ...
    if (g_MySelf.m_btHorse <> 0) and g_ClientConfig.boHorseRun3Grid then begin
      FIsRun3Grid := (g_MySelf.m_btHorse <> 0) and g_ClientConfig.boHorseRun3Grid;

      while True do begin
        if not StartFind then Break;
        if I >= Length(WalkPath) then Break;
        if nStep >= 3 then begin
          nDir0 := GetNextDirection(WalkPath[I - 3].x, WalkPath[I - 3].y, WalkPath[I - 2].x, WalkPath[I - 2].y);
          ...
          if (nDir0 = nDir1) and (nDir1 = nDir2) then begin
            WalkPath[I - 2].x := -1;
            WalkPath[I - 2].y := -1;
...
begin
  ClientRect := Bounds(0, 0, FWidth, FHeight);
...
var
  ...
  begin
    ...
    for Y := Low(Result) to High(Result) do begin
      if not StartFind then Exit;
      for X := Low(Result[Y]) to High(Result[Y]) do begin
        if not StartFind then Exit;
...
procedure TLegendMap.FindPath(StopX, StopY:Integer; ExcludeMonster:Boolean);
var
  ErrCode:Integer;
begin
  ErrCode := 0;
  try
    ...
    on E:Exception do begin
      DebugOutStr('TLegendMap.FindPath1 Error; Code =' + IntToStr(ErrCode));
      DebugOutStr(E.Message);
    end;
  end;
end;

procedure TLegendMap.Find(StartX, StartY, StopX, StopY:Integer; ExcludeMonster:Boolean);
begin
  with TFindPathThread.Create(StartX, StartY, StopX, StopY) do begin
    FExcludeMonster := ExcludeMonster;
  end;
end;

procedure TLegendMap.FindPath(StartX, StartY, StopX, StopY:Integer; PathSpace:Integer; ExcludeMonster:Boolean);
var
  ErrCode:Integer;
begin
  ErrCode := 0;
  try
    ...
    on E:Exception do begin
      DebugOutStr('TLegendMap.FindPath2 Error; Code =' + IntToStr(ErrCode));
      DebugOutStr(E.Message);
    end;
  end;
end;
...
begin
  ...
  if (X < 0) or (X >= ClientRect.Right - ClientRect.Left) or (Y < 0) or (Y >= ClientRect.Bottom - ClientRect.Top) then
    ...
    if not FExcludeMonster then begin
      if PlayScene.NewCanWalkEx(nX, nY) then
        Result := 4
      else
        Result := -1;
    end else begin
      if PlayScene.NewCanWalkEx_2(nX, nY) then
        Result := 4
      else
        Result := -1;
    end;
