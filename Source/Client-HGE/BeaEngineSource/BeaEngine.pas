...
function sprintf(buf:Pointer; format:PAnsiChar {args}):Integer; cdecl;
varargs;
...
public name _PU + 'memset';
begin
  Result := memset(dest, val, count);
end;

function _sprintf(buf:Pointer; format:PAnsiChar {args}):Integer; cdecl; {varargs;}
public name _PU + 'sprintf';
begin
  Result := sprintf(buf, format);
end;

function _strcmp(s1:PAnsiChar; s2:PAnsiChar):Integer; cdecl;
public name _PU + 'strcmp';
begin
  Result := strcmp(s1, s2);
end;

function _strcpy(dest, src:PAnsiChar):PAnsiChar; cdecl;
public name _PU + 'strcpy';
begin
  Result := strcpy(dest, src);
end;

function _strlen(s:PAnsiChar):size_t; cdecl;
public name _PU + 'strlen';
begin
  Result := strlen(s);
end;
{$ENDIF}
