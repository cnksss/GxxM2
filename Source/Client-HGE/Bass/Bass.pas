...
var
  ...
  InitializeBassDLLOK: Boolean = False;
  ...
  BASS_ErrorGetCode: function: LongInt; stdcall;
  BASS_GetDeviceInfo: function(device: DWORD; var info: BASS_DEVICEINFO): BOOL; stdcall;
  BASS_Init: function(device: LongInt; freq, flags: DWORD; win: HWND; clsid: PGUID): BOOL; stdcall;
  ...
  BASS_SetEAXParameters: function(env: LongInt; vol, decay, damp: FLOAT): BOOL; stdcall;
  ...
  BASS_RecordInit: function(device: LongInt): BOOL; stdcall;
  ...
  BASS_RecordGetInputName: function(input: LongInt): PAnsiChar; stdcall;
  BASS_RecordSetInput: function(input: LongInt; flags: DWORD; volume: FLOAT): BOOL; stdcall;
  BASS_RecordGetInput: function(input: LongInt; var volume: FLOAT): DWORD; stdcall;
  ...
  BASS_ChannelSet3DAttributes: function(handle: DWORD; mode: LongInt; min, max: FLOAT; iangle, oangle, outvol: LongInt): BOOL; stdcall;
  ...
  BASS_ChannelSetDSP: function(handle: DWORD; proc: DSPPROC; user: Pointer; priority: LongInt): HDSP; stdcall;
  ...
  BASS_ChannelSetFX: function(handle, type_: DWORD; priority: LongInt): HFX; stdcall;
...
implementation
uses
  Classes, SysUtils, BassDllFiles, DLLLoader;
var
  BassDllLoader: TDLLLoader;
...
var
  ...
  procedure Fetch(var Proc: Pointer; const ProcName: String);
  begin
    Proc := BassDllLoader.FindExport(ProcName);
...
begin
  if (BassDllLoader <> nil) then begin
    Result := True;
    Exit;
  end;
  ...
  BassDllLoader := TDLLLoader.Create;
  BassDllLoader.Load(Stream);
  Stream.Free;
  ...
  try
    ...
    Fetch(@BASS_SetConfigPtr, 'BASS_SetConfigPtr');
    Fetch(@BASS_GetConfigPtr, 'BASS_GetConfigPtr');
    Fetch(@BASS_GetVersion, 'BASS_GetVersion');
    Fetch(@BASS_ErrorGetCode, 'BASS_ErrorGetCode');
    Fetch(@BASS_GetDeviceInfo, 'BASS_GetDeviceInfo');
    ...
    Fetch(@BASS_SampleSetData, 'BASS_SampleSetData');
    Fetch(@BASS_SampleGetData, 'BASS_SampleGetData');
    ...
    Fetch(@BASS_SampleGetChannels, 'BASS_SampleGetChannels');
    ...
    Fetch(@BASS_StreamPutData, 'BASS_StreamPutData');
    Fetch(@BASS_StreamPutFileData, 'BASS_StreamPutFileData');

    Fetch(@BASS_RecordGetDeviceInfo, 'BASS_RecordGetDeviceInfo');
    ...
    Fetch(@BASS_ChannelFlags, 'BASS_ChannelFlags');
    Fetch(@BASS_ChannelUpdate, 'BASS_ChannelUpdate');
    Fetch(@BASS_ChannelLock, 'BASS_ChannelLock');
    Fetch(@BASS_ChannelPlay, 'BASS_ChannelPlay');
    Fetch(@BASS_ChannelStop, 'BASS_ChannelStop');
    Fetch(@BASS_ChannelPause, 'BASS_ChannelPause');
    Fetch(@BASS_ChannelSetAttribute, 'BASS_ChannelSetAttribute');
    Fetch(@BASS_ChannelGetAttribute, 'BASS_ChannelGetAttribute');
    Fetch(@BASS_ChannelSlideAttribute, 'BASS_ChannelSlideAttribute');
    ...
    Fetch(@BASS_FXSetParameters, 'BASS_FXSetParameters');
    Fetch(@BASS_FXGetParameters, 'BASS_FXGetParameters');
    Fetch(@BASS_FXReset, 'BASS_FXReset');

    if (not OK) then
      FinalizeBassDLL;
    InitializeBassDLLOK := OK;
    Result := OK;
  finally
...
begin
  {if (BassHandle <> 0) then begin
    ...
    BassHandle := 0;
    InitializeBassDLLOK := False;
  end;}
  InitializeBassDLLOK := False;
  if BassDllLoader <> nil then begin
    BASS_Stop;
    BASS_Free;
    FreeAndNiL(BassDllLoader);
  end;
end;
