    ...
    begin
      ...
      begin
        ...
        begin
         ...
         else if (Source.FWidth - X > 5) and ((GetPixel(X) <> GetPixel(X + 2)) or (GetPixel(X + 1) <> GetPixel(X + 3))) and
    ...
    begin
      ...
      begin
        ...
        begin
         ...
         else if (Source.FWidth - X > 2) and (Src^ <> PByte(Integer(Src) + 1)^) and (PByte(Integer(Src) + 1)^ = PByte(Integer(Src) + 2)^) then
        begin
          {  Encoding mode }
          AllocByte^ := 1;
    ...
    case GetObject(Source.Handle, SizeOf(Data), @Data) of
      ...
      SizeOf(TDIBSection):
        begin
          ...
          else  if DIBSectionRec^.dsBm.bmBitsPixel > 8 then
          begin
...
begin
  ...
  try
    ...
    else  if (Temp.BitCount <= 8) and (BitCount > 8) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else  if ifx > Pred(Width) then
        ifx := ifx mod Width;
      if ify < 0 then
        ify := Pred(Height) - (-ify mod Height)
      else  if ify > Pred(Height) then
        ify := ify mod Height;
      total_red := 0.0;
      total_green := 0.0;
      total_blue := 0.0;
...
begin
  ...
  begin
    ...
    begin
      ...
      else  if ifrom_x > Pred(Width) then
        ifrom_x := ifrom_x mod Width;
      if ifrom_y < 0 then
        ifrom_y := Pred(Height) - (-ifrom_y mod Height)
      else  if ifrom_y > Pred(Height) then
