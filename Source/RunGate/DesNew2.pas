...
procedure DecryptDes_New2(const Indata; var Outdata; Size: Longint; const Key: AnsiString);
...
{$I VMProtectBegin.inc}
  ...
  begin
    ...
    begin
      ...
      // c -= a; c -= b; c ^= b >> 13;
      c := c - a;   c := c - b;   c := c xor (b shr 19);

      // a -= b; a -= c; a ^= c >> 12;
      a := a - b;   a := a - c;   a := a xor (c shr 12);

      // b -= c; b -= a; b ^= a << 16;
      b := b - c;   b := b - a;   b := b xor (a shl 11);
  ...
  begin
    ...
    Move(Temp, Chain, BS);
    p1 := pointer(DWORD(p1) + BS);
    p2 := pointer(DWORD(p2) + BS);
  end;
