...
type
  ...
  public
    ...
    // 一直读取直到发现字符,专用于mir2网关 chongchong 2014-06-12
    function ReadBufferWhileFindChar(const buf: PAnsiChar; c: AnsiChar): Cardinal;
  ...
  function GetTotalMemBytes: Int64;
...
var
  ...
  TotalMemLocker: TCriticalSection;

function GetTotalMemBytes: Int64;
begin
  Result := TotalMemBytes;
end;

procedure AddMemory(nSize: Integer);
begin
  TotalMemLocker.Enter;
  TotalMemBytes := TotalMemBytes + nSize;
  TotalMemLocker.Leave;
end;

procedure DelMemory(nSize: Integer);
begin
  TotalMemLocker.Enter;
  TotalMemBytes := TotalMemBytes - nSize;
  TotalMemLocker.Leave;
...
begin
  if SuperBigMPool = nil then
    SuperBigMPool := TDxMemoryPool.Create(2048, 30, MB_SpBig, 30);
...
begin
  if LargeMPool = nil then
    LargeMPool := TDxMemoryPool.Create(4096, 30, MB_Large, 30);
  Result := LargeMPool;
end;
...
begin
  if SPLargeMPool = nil then
    SPLargeMPool := TDxMemoryPool.Create(4096 * 4, 30, MB_SpLarge, 30);                            //16KB
  Result := SPLargeMPool;
end;
...
begin
  if NormalMPool = nil then
    NormalMPool := TDxMemoryPool.Create(640, 30, MB_Normal, 30);
  Result := NormalMPool;
end;
...
begin
  if SmallMPool = nil then
    SmallMPool := TDxMemoryPool.Create(128, 30, MB_Small, 30);                                     //小内存块
  Result := SmallMPool;
end;
...
begin
  if BigMPool = nil then
    BigMPool := TDxMemoryPool.Create(1024, 30, MB_Big, 30);                                        //大内存块
  Result := BigMPool;
end;
{ TDxMemoryPool }
...
begin
  ...
  try
    ...
    begin
      ...
      FreeMem(p^.Memory);
      FreeMem(p);
      DelMemory(SizeOf(TMemoryBlock) + FBlockSize);
      p := tmp;
    end;
    p := FUnUseHead;
    while p <> nil do
...
begin
  ...
  begin
    ...
    AddMemory(SizeOf(TMemoryBlock) + FBlockSize);
...
begin
  ...
  begin
    ...
    case FMemBlockType of
      ...
      BSize := BigMemoryPool.FBlockSize;
    end;
    while tmp <> FLast do
    begin
      tmp^.DataLen := BSize;
  ...
  begin
    ...
    begin
      ...
      if Windex = FMemBlockCount - 1 then
        ...
        BufList.ReadBuffer(tmpBlock^.Memory, MPool.FBlockSize);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Stream.ReadBuffer(tmpBlock^.Memory^, MPool.FBlockSize);
      tmpBlock := tmpBlock^.NextEx;
    end;
    FPosition := 0;
    FCurBlock := FHead;
  ...
  begin
    ...
    begin
      if tmp <> FLast then
        ...
        Stream.WriteBuffer(tmp^.Memory^, MPool.FBlockSize - FCapacity + FSize);
      tmp := tmp^.NextEx;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    case FLast^.BlockType of
      ...
      BlockSize := BigMemoryPool.FBlockSize;
    end;
...
begin
  ...
  begin
    ...
    case lvFreeBuf^.BlockType of
      ...
      BigMemoryPool.FreeMemoryBlock(lvFreeBuf);
    end;
  end;
  FHead := nil;
  FLast := nil;
...
function TBufferLink.ReadBufferWhileFindChar(const buf: PAnsiChar; C: AnsiChar): Cardinal;
var
  boIsFoundChar: Boolean;
  lvBuf: PMemoryBlock;
  I: Integer;
  Chr: AnsiChar;
  lvPosition, l, lvReadCount, lvRemain, lvIndex: Cardinal;
begin
  Result := 0;
  lvReadCount := 0;
  lvBuf := FRead;
  ...
  begin
    ...
    begin
      ...
      for I := lvPosition to lvBuf.DataLen - 1 do
      begin
        Chr := PAnsiChar(NativeUInt(lvBuf.Memory) + I)^;
        if Chr = C then
        begin
          boIsFoundChar := True;
          lvIndex := I;
      ...
      if boIsFoundChar then
        lvRemain := lvIndex - lvPosition + 1
      else
      begin
        lvRemain := lvBuf.DataLen - lvPosition;
        if lvRemain = 0 then Break;
      end;
      ...
      begin
        l := InnerReadBuf(lvBuf, lvPosition, Pointer(NativeUInt(buf) + lvReadCount), lvRemain);
        if l <> lvRemain then
          raise Exception.Create('TBufferLink.ReadBufferWhileFindChar Error');
      end
      else
        l := lvRemain;

      // 找到字符 chongchong 2014-06-12
      if boIsFoundChar then
      begin
        //读完
        inc(lvReadCount, l);
...
initialization
  begin
    TotalMemBytes := 0;
    TotalMemLocker := TCriticalSection.Create;
  end;
