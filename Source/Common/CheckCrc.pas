...
function Crc32(buf: PByte; len: Integer): Cardinal; //实现计算

procedure make_crc_table; //动态创建CRC表

var DynamicMakeCRCTableFlage: BOOL = False; //是否动态产生CRC表
...
begin
  if (buf = nil) or (Len <= 0) then begin
    ...
    begin
        {一次处理8个再循环来加快处理速度}
      nCrc := crc_table[(nCrc xor Cardinal(buf^)) and $FF] xor (nCrc shr 8);
      inc(buf);
      nCrc := crc_table[(nCrc xor Cardinal(buf^)) and $FF] xor (nCrc shr 8);
      inc(buf);
      nCrc := crc_table[(nCrc xor Cardinal(buf^)) and $FF] xor (nCrc shr 8);
