...
type
  ...
  public
    ...
    procedure SortString(nMin, nMax: NativeInt);
    function GetIndex(sName: string): NativeInt;
    function AddRecord(sName: string; nIndex: NativeInt): Boolean;
    procedure Lock;
    procedure UnLock;
  published
...
var
  QuickID: pTQuickID;
  ChrList: TList;
  nLow, nHigh, nMed, n1C, n20: NativeInt;
begin
  New(QuickID);
...
begin
  ...
  begin
    ...
    if QuickID.sChrName = sChrName then
    begin
      Dispose(QuickID);
      ChrList.Delete(i);
      break;
...
begin
  ...
  begin
    if CompareStr(sAccount, Self.Strings[0]) = 0 then
    begin
...
function TQuickList.GetIndex(sName: string): NativeInt;
// 0x0045B498
var
  nLow, nHigh, nMed, nCompareVal: NativeInt;
begin
  Result := -1;
...
procedure TQuickList.SortString(nMin, nMax: NativeInt);
// 0x0045AF78
var
  ntMin, ntMax: NativeInt;
  s18: string;
begin
  if Self.Count > 0 then
    while (True) do
...
function TQuickList.AddRecord(sName: string; nIndex: NativeInt): Boolean;
// 0x0045B0D0
var
  nLow, nHigh, nMed, nCompareVal: NativeInt;
begin
