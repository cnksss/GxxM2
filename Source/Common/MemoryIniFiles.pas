...
interface
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, IniFiles, SDK;
type
  ...
  TIniValueList = class(TValueList)
  public
    function GetIndex(const AName: string): Integer; override;
  end;
  ...
  public
    ...
    procedure SaveToList(const SaveList: TStrings);
...
function TIniValueList.GetIndex(const AName: string): Integer;
var
  I: Integer;
begin
  Result := -1;
  ...
  begin
    if CompareStr(AName, Self.Names[I]) = 0 then
    begin
      Result := I;
      break;
    end;
...
procedure TMemoryIniFile.SaveToList(const SaveList: TStrings);
var
  I, II: Integer;
  Strings: TValueList;
  Section: string;
...
begin
  ...
  begin
    ...
    // nPos := FastCharPos(LineText, ']', 1);
    nPos := Pos(']', LineText);
    if (LineText <> '') and (LineText[1] = '[') and (nPos > 0) then
    begin                                                                                           // (Pos(']', LineText) > 0)
      Strings := TIniValueList.Create;
      Strings.OnChange := Changed;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Strings.AddObject(sName, sValue, TObject(0));
        end;
      end;
    end;
  end;
...
procedure TMemoryIniFile.ReadSection(const Section: string; Strings: TStrings);
var
  nIndex: Integer;
...
begin
  ...
  if (nIndex >= 0) then
  begin
    ValueList := TValueList(Sections.Objects[nIndex]);
    Strings.AddStrings(ValueList);
  end;
end;
...
begin
  Result := Default;
  if (Section >= 0) and (Section < Sections.Count) then
  begin
