...
function MemoryLoadLibrary(Data: Pointer): TMemoryModule; stdcall;
// return value is nil if function fails
...
begin
  ...
  begin
    ...
    while thunkRef^ <> 0 do begin
      if IMAGE_SNAP_BY_ORDINAL(thunkRef^) then begin
...
begin
  ...
  try
    ...
    Result := THandle(Module);
  except
    // cleanup
    MemoryFreeLibrary(THandle(Module));
    Exit;
  end;
end;
...
begin
  ...
  if (exportDir.NumberOfNames = 0) or (exportDir.NumberOfFunctions = 0) then begin
    SetLastError(ERROR_PROC_NOT_FOUND);
    Exit;
  end;
  ...
  Idx := -1;
  for i := 0 to exportDir.NumberOfNames - 1 do begin
    if StrComp(Name, PAnsiChar(PByte(CodeBase) + nameRef^)) = 0 then begin
      Idx := ordinal^;
      Break;
    end;
    Inc(nameRef);
  ...
  if (DWORD(Idx) > exportDir.NumberOfFunctions) then begin
    SetLastError(ERROR_PROC_NOT_FOUND);
    Exit;
  end;
...
begin
  ...
  if mmodule.Initialized then begin
  ...
  if Length(mmodule.Modules) <> 0 then begin
    // free previously opened libraries
    for i := 0 to mmodule.NumModules - 1 do begin
        if mmodule.Modules[i] <> 0 then begin
