...
{         Fixed By PiaoYun/P.Y.G - 2012.12       }
{                                                }
{************************************************}
...
begin
  Result := PPointer(NativeInt(AClass) + (Index * 4))^
end;
...
begin
  PatchAddress := PPointer(NativeInt(AClass) + (Index * 4));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (JmpTo < PtStart) or (JmpTo > (PtEnd + 1)) then
          begin
            Delta := (OldPt + NativeInt(AsmSize) - Pt + (OpBytes - 1));
