...
uses
  Windows, Classes, System.SysUtils, Graphics, SynEdit, SynEditTypes, SynEditHighlighter;
...
procedure TSearchTextHightlighterSynEditPlugin.AfterPaint(ACanvas: TCanvas; const AClip: TRect; FirstLine, LastLine: Integer);
var
  SearchText, SearchResultText: AnsiString;
  Pt: TPoint;
  Rct: TRect;
  OldFont: TFont;
  LineIndex, Count, ItemIndex: Integer;
  CurrCoord, tmpCoord: TBufferCoord;
begin
  inherited;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        tmpCoord := Editor.BlockBegin;
        if CompareMem(@CurrCoord, @tmpCoord, SizeOf(TBufferCoord)) then
          Continue;
        ...
        Rct := Rect(Pt.X, Pt.Y, Pt.X + Editor.CharWidth * Length(SearchResultText), Pt.Y + Editor.LineHeight);
