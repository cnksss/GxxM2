...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, ObjNpc, SpinEditEx, NpcCommon,
  Vcl.Samples.Spin;
...
type
  TfrmConfigMerchant = class(TForm)
    ...
    procedure chkCreateHeroClick(Sender: TObject);
    procedure chkBuHeroClick(Sender: TObject);
...
begin
  if Application.MessageBox(PChar('是否确认清除NPC临时数据？'), '确认信息', MB_YESNO + MB_ICONQUESTION) = mrYes then
  begin
    ClearMerchantData();
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      SaveList.Add(Script + #9 + Merchant.m_sMapName + #9 + IntToStr(Merchant.m_nCurrX) + #9 + IntToStr(Merchant.m_nCurrY) + #9 +
        Merchant.m_sCharName + #9 + IntToStr(Merchant.m_nFlag) + #9 + IntToStr(Merchant.m_wAppr) + #9 + sIsCastle + #9 + sCanMove
        + #9 + IntToStr(Merchant.m_dwMoveTime))
    end;
...
var
  ...
  Merchant: TMerchant;
begin
  UserEngine.m_MerchantList.LockR(12);
  try
    for I := 0 to UserEngine.m_MerchantList.Count - 1 do
    begin
      Merchant := TMerchant(UserEngine.m_MerchantList.Items[I]);
...
var
  ...
  Merchant: TMerchant;
begin
  UserEngine.m_MerchantList.LockR(13);
  try
    for I := 0 to UserEngine.m_MerchantList.Count - 1 do
    begin
      Merchant := TMerchant(UserEngine.m_MerchantList.Items[I]);
      if (Merchant.m_sMapName = '0') and (Merchant.m_nCurrX = 0) and (Merchant.m_nCurrY = 0) then
        Continue;
      ListBoxMerChant.Items.AddObject(Merchant.m_sCharName + ' - ' + Merchant.m_sMapName + ' (' + IntToStr(Merchant.m_nCurrX) +
        ':' + IntToStr(Merchant.m_nCurrY) + ')', Merchant);
    end;
  finally
    UserEngine.m_MerchantList.UnLockR;
  end;
...
begin
  ...
  if SelMerchant.m_boBuy then
    LineText := LineText + sNF_Buy + ' ';
  if SelMerchant.m_boSell then
    LineText := LineText + sNF_Sell + ' ';
  if SelMerchant.m_boMakeDrug then
    LineText := LineText + sNF_MakedUrg + ' ';
  if SelMerchant.m_boStorage then
    LineText := LineText + sNF_Storage + ' ';
  if SelMerchant.m_boGetback then
    LineText := LineText + sNF_Getback + ' ';
  if SelMerchant.m_boUpgradenow then
    LineText := LineText + sNF_UpgradeNow + ' ';
  if SelMerchant.m_boGetBackupgnow then
    LineText := LineText + sNF_GetBackupgNow + ' ';
  if SelMerchant.m_boRepair then
    LineText := LineText + sNF_Repair + ' ';
  if SelMerchant.m_boS_repair then
    LineText := LineText + sNF_SuperRepair + ' ';
  if SelMerchant.m_boSendmsg then
    LineText := LineText + sNF_SendMsg + ' ';
  if SelMerchant.m_boArmRemoveStone then
    LineText := LineText + sNF_ArmRemoveStone + ' ';
...
begin
  ...
  if SelMerchant.m_boSendmsg then
    LineText := LineText + sNF_SendMsg + ' ';
  if SelMerchant.m_boCreateHeroName then
    LineText := LineText + sNF_CreateHero + ' ';
  if SelMerchant.m_boBuHero then
    LineText := LineText + sNF_CreateDeputy + ' ';
  if SelMerchant.m_boArmRemoveStone then
    LineText := LineText + sNF_ArmRemoveStone + ' ';
  LineText := LineText + ')';
...
procedure TfrmConfigMerchant.chkCreateHeroClick(Sender: TObject);
begin
  { TODO -ochongchong -c新增 : 工具-交易npc配制 增加创建英雄 【2013-07-23】 }
  if not boOpened or (SelMerchant = nil) then
    Exit;
...
procedure TfrmConfigMerchant.chkBuHeroClick(Sender: TObject);
begin
  { TODO -ochongchong -c新增 : 工具-交易npc配制 增加副将英雄 【2013-07-23】 }
  if not boOpened or (SelMerchant = nil) then
    Exit;
  SelMerchant.m_boBuHero := chkBuyHero.Checked;
  ModValue();
  ChangeScriptAllowAction();
end;

procedure TfrmConfigMerchant.btnSearchClick(Sender: TObject);
var
  I: Integer;
  S, S2: string;
begin
  ...
  for I := 0 to ListBoxMerChant.Items.Count - 1 do
  begin
    S2 := LowerCase(ListBoxMerChant.Items[I]);
    if Pos(S, S2) > 0 then
    begin
      ListBoxMerChant.ItemIndex := I;
      ListBoxMerChant.OnClick(ListBoxMerChant);
    end;
  end;
end;
