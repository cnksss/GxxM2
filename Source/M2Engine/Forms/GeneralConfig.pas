...
begin
  ...
  GatePort := StrToIntDef(Trim(EditGatePort.Text), -1);
  IDSAddr := Trim(EditIDSAddr.Text);
  IDSPort := StrToIntDef(Trim(EditIDSPort.Text), -1);
  DBAddr := Trim(EditDBAddr.Text);
  DBPort := StrToIntDef(Trim(EditDBPort.Text), -1);
  LogServerAddr := Trim(EditLogServerAddr.Text);
  LogServerPort := StrToIntDef(Trim(EditLogServerPort.Text), -1);
...
begin
  ...
  edtBoxsDir.Text := g_Config.sBoxsDir;                                                             // 宝箱目录 piaoyun 2013-08-22
...
var
  GameName, DBName, SqliteDBName: string;
...
begin
  ...
  ServerIndex := StrToIntDef(Trim(EditServerIndex.Text), -1);
  ServerNumber := StrToIntDef(Trim(EditServerNumber.Text), -1);
  ServiceMode := CheckBoxServiceMode.Checked;
  TestServer := CheckBoxTestServer.Checked;
  TestLevel := StrToIntDef(Trim(EditTestLevel.Text), -1);
  TestGold := StrToIntDef(Trim(EditTestGold.Text), -1);
  TestUserLimit := StrToIntDef(Trim(EditTestUserLimit.Text), -1);
  UserFull := StrToIntDef(Trim(EditUserFull.Text), -1);

  DBName := Trim(EditDBName.Text);
  SqliteDBName := Trim(edtSqlite.Text);
  ...
  begin
    if not FileExists(SqliteDBName) then
    begin
      Application.MessageBox('数据库文件设置错误！', '错误信息', MB_OK + MB_ICONERROR);
      edtSqlite.SetFocus;
      Exit;
    end;
  end;
  ...
  Config.WriteBool('Setup', 'UseSqliteDB', g_boUseSqliteDB);
  Config.WriteString('Server', 'DBName', g_sDBName);
  Config.WriteString('Server', 'SqliteDBName', g_sSqliteDBName);

  Config.WriteBool('Server', 'ShowBlockIPLog', g_boShowBlockIPLog);
...
var
  GuildDir, GuildFile, VentureDir, ConLogDir, CastleDir, EnvirDir, MapDir, NoticeDir, PlugDir, BoxsDir: string;
begin
  ...
  BoxsDir := Trim(edtBoxsDir.Text);                                                                 // 宝箱目录 piaoyun 2013-08-22
  ...
  if not DirectoryExists(BoxsDir) or (BoxsDir[Length(BoxsDir)] <> '\') then
  begin
  ...
  g_Config.sBoxsDir := BoxsDir;                                                                     // 宝箱目录 piaoyun 2013-08-22
...
procedure TfrmGeneralConfig.rbBDEClick(Sender: TObject);
begin
  EditDBName.Enabled := rbBDE.Checked;
  edtSqlite.Enabled := not rbBDE.Checked;
end;

procedure TfrmGeneralConfig.rbSqliteClick(Sender: TObject);
begin
  EditDBName.Enabled := rbBDE.Checked;
  edtSqlite.Enabled := not rbBDE.Checked;
end;
