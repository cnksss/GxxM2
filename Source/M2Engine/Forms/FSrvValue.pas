...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, Buttons, M2Share, SpinEditEx,
  Vcl.Samples.Spin;
...
type
  TFrmServerValue = class(TForm)
    ...
    Label9: TLabel;
    Label10: TLabel;
    EGateLoad: TSpinEditEx;
    EAvailableBlock: TSpinEditEx;
    ECheckBlock: TSpinEditEx;
    ESendBlock: TSpinEditEx;
    Label11: TLabel;
    Label12: TLabel;
    Label13: TLabel;
    Label14: TLabel;
    ...
    GroupBox3: TGroupBox;
    Label15: TLabel;
    EditZenMonRate: TSpinEditEx;
    Label16: TLabel;
    EditProcessTime: TSpinEditEx;
    EditZenMonTime: TSpinEditEx;
    Label17: TLabel;
    Label18: TLabel;
    ButtonDefault: TButton;
    Label19: TLabel;
    EditProcessMonsterInterval: TSpinEditEx;
    CheckBoxSendCompressDataToRunGate: TCheckBox;
    ...
    procedure CheckBoxSendCompressDataToRunGateClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
    procedure ModValue();
...
begin
  ...
  Config.WriteBool('Setup', 'SendCompressDataToRunGate2', g_Config.boSendCompressDataToRunGate);

  //Config.WriteBool('Setup', 'SendOptimizeDataToRunGate', g_Config.boSendOptimizeDataToRunGateTemp);
...
begin
  ...
  EditProcessTime.Hint := '处理怪物的时间间隔，此设置数字越大，怪物行动越慢。数字越小，怪物行动越灵活，CPU占用越高。默认值250';
  EditProcessMonsterInterval.Hint := '空闲时处理怪物检测次数，数字越大怪物运行越迟钝。默认=3';
end;
...
begin
  if not boOpened then
    Exit;
  g_Config.nSendBlock := Max(10, ESendBlock.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_Config.nCheckBlock := ECheckBlock.Value;
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_dwHumLimit := Min(150, EHum.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_dwMonLimit := Min(150, EMon.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_dwZenLimit := Min(150, EZen.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_dwSocLimit := Min(150, ESoc.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_dwNpcLimit := Min(150, ENpc.Value);
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  g_Config.nAvailableBlock := Max(10, EAvailableBlock.Value);
  ModValue();
end;
...
begin
  ...
  CheckBoxSendCompressDataToRunGate.Checked := g_Config.boSendCompressDataToRunGate;

  // chkSendOptimizeDataToRunGate.Checked := g_Config.boSendOptimizeDataToRunGateTemp;
end;
...
begin
  ...
  nDecLimit := 20;
  g_dwNpcLimit := 5;
  g_Config.nSendBlock := 4000;
  g_Config.nCheckBlock := 10000;
  g_Config.nAvailableBlock := 8000;
  g_Config.nGateLoad := 0;
  g_Config.boViewHackMessage := False;
  g_Config.boViewAdmissionFailure := False;
  g_Config.boSendCompressDataToRunGate := False;
  g_Config.boIsOldClient := True;
  g_Config.nMonGenRate := 10;
  g_Config.dwRegenMonstersTime := 200;
  g_Config.dwProcessMonstersTime := 250;
  g_Config.nProcessMonsterInterval := 3;
  RefShow();
end;
...
procedure TFrmServerValue.CheckBoxSendCompressDataToRunGateClick(Sender: TObject);
begin
  if not boOpened then
    Exit;
  g_Config.boSendCompressDataToRunGate := CheckBoxSendCompressDataToRunGate.Checked;
  ModValue();
end;
