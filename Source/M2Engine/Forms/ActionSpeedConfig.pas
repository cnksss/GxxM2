...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, Spin, SpinEditEx;
...
type
  TfrmActionSpeed = class(TForm)
    ...
    GroupBox3: TGroupBox;
    Label15: TLabel;
    EditRunLongHitIntervalTime: TSpinEditEx;
    GroupBox2: TGroupBox;
    Label2: TLabel;
    EditActionIntervalTime: TSpinEditEx;
    CheckBoxControlActionInterval: TCheckBox;
    CheckBoxControlRunLongHit: TCheckBox;
    GroupBox4: TGroupBox;
    Label1: TLabel;
    EditRunHitIntervalTime: TSpinEditEx;
    CheckBoxControlRunHit: TCheckBox;
    GroupBox5: TGroupBox;
    Label3: TLabel;
    EditWalkHitIntervalTime: TSpinEditEx;
    ...
    GroupBox6: TGroupBox;
    Label4: TLabel;
    EditRunMagicIntervalTime: TSpinEditEx;
...
begin
  ...
  if (MessageBox(Handle, PChar(sExitMsg), PChar(sExitMsgTitle), MB_YESNO + MB_ICONQUESTION) = IDYES) then
  begin
    Close;
  end;
end;
...
resourcestring
  ...
  sExitMsgTitle = '确认信息';
begin
  if Application.MessageBox(PChar(sExitMsg), PChar(sExitMsgTitle), MB_YESNO + MB_ICONQUESTION) <> IDYES then
  begin
    Exit;
  end;
  boOpened := False;
...
procedure TfrmActionSpeed.CheckBoxControlActionIntervalClick(Sender: TObject);
var
  boStatus: Boolean;
begin
...
procedure TfrmActionSpeed.EditRunLongHitIntervalTimeChange(Sender: TObject);
begin
  if not boOpened then
    Exit;
