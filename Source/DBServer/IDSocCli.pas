...
type
  ...
  private
    ...
    m_dwKeepAlivePacketTick: LongWord;
...
begin
  ...
  m_dwKeepAlivePacketTick := GetTickCount;
end;
...
begin
  ...
  begin
    ...
    begin
      if SameText(GlobaSessionInfo.sAccount, sAccount) and (GlobaSessionInfo.nSessionID = nSessionID) then
      begin
        Result := True;
        Break;
      end;
...
begin
  ...
  begin
    ...
    if GlobaSessionInfo <> nil then
    begin
      if SameText(GlobaSessionInfo.sAccount, sAccount) then
      begin
...
begin
  ...
  GlobaSessionInfo.n24 := StrToIntDef(s14, 0);
  GlobaSessionInfo.boStartPlay := False;
  GlobaSessionInfo.boLoadRcd := False;
...
begin
  ...
  begin
    ...
    if GlobaSessionInfo <> nil then
    begin
      if (GlobaSessionInfo.nSessionID = nSessionID) and SameText(GlobaSessionInfo.sAccount, sAccount) then
      begin
        GlobaSessionList.Delete(I);
        Dispose(GlobaSessionInfo);
...
begin
  if GetTickCount - m_dwKeepAlivePacketTick > 3000 then
  begin
    m_dwKeepAlivePacketTick := GetTickCount;
    if IDSocket.Socket.Connected then
    begin
...
begin
  ...
  begin
    ...
    if GlobaSessionInfo <> nil then
    begin
      if SameText(GlobaSessionInfo.sAccount, sAccount) and (GlobaSessionInfo.sIPaddr = sIPaddr) then
      begin
        Result := True;
        Break;
      end;
