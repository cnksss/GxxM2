...
uses
  Windows, SysUtils, Messages, WinSock2, SHSocket, ThreadPool, MemPool,
    CommCtrl,
...
type
  ...
  TOnUserEnter = procedure(Sender: TUserManager; const UserOBJ: PUserOBJ) of
    object;
  TOnUserLeave = procedure(Sender: TUserManager; const UserOBJ: PUserOBJ) of
    object;
  ...
  protected
    procedure HandleException(const PTRMsgError: string; Level: Integer);
  public
    ...
    property OnInitCompPortFinished: TOnCompPortEvent read
      FOnInitCompPortFinished write FOnInitCompPortFinished;
    property OnCleanupComPortEnd: TOnCompPortEvent read FOnCleanupCompPortEnd
      write FOnCleanupCompPortEnd;
    ...
    procedure FreeAcceptEx(const IOCPData: PIOCPCommObj; const ListenSocket:
      TSocket);
    //轮询所有的SOCKET是否有效
  ...
  TOnReadEvent = procedure(UserData: TObject; const Buffer: PChar; var BufLen:
    DWORD; var Succeed: BOOL) of object;
  ...
  public
    ...
    function AddLoginUser(const IOCPData: PLoginObj; var ReaderOBJ: PRecvObj):
      Boolean; //如果IOLen=0 则登陆命令传入到了数据库服务器，否则还需要继续接受登陆请求
    procedure DeleteLoginUser(IOCPData: PIOCPCommObj);
      //在工作过程中SOCKET发生错误，这个时候删除该套接字
    ...
    function HandleReadData(const IOCPData: PIOCPCommObj; const IOLen: DWORD):
      BOOL;
    constructor Create(MaxUser: Integer; SendSocket: TIOCPWriter; UserManager:
      TUserManager);
    destructor Destroy; override;
  end;

  TOnBroadCastEvent = procedure(UserOBJ: PUserOBJ; wParam, lParam: DWORD) of
    object;
  ...
  protected
    ...
    procedure DeleteLoginUser(IOCPData: PIOCPCommObj);
    function AddLoginUser(IOCPData: PIOCPCommObj; var WriterOBJ: PIOCPCommObj):
      Boolean;
  public
    property SendQueue: TSendQueue read FSendQueue;
    property CurrentUser: Integer read GetCurrentUser;
    property OnBroadcastEvent: TOnBroadCastEvent read FOnBroadCastEvent write
      FOnBroadCastEvent;
    procedure PostAllUserSend(const Buffer: PChar; const BufLen: DWORD);
    procedure BroadUserMsg(wParam, lParam: Integer; OnBroadcastEvent:
      TOnBroadCastEvent);
    function SendData(const WriterOBJ: PIOCPCommObj; const Buffer: PChar; const
      size: UINT): Boolean;
  ...
  protected
    ...
    function HandleRecvEvent(const IOCPData: PIOCPCommObj; const IOLength:
      DWORD): Boolean;
    function HandleSendEvent(const IOCPData: PIOCPCommObj; const IOLength:
      DWORD): Boolean;
    procedure HandleAllError(const IOCPData: PIOCPCommObj; const IOKey: DWORD);
  public
    constructor Create(SendSocket: TIOCPWriter; RecvSocket: TIOCPReader;
      AcceptExSocket: TIOCPAccepter; IOCPPort: THANDLE);
    destructor Destroy; override;
  end;
...
procedure TAcceptExWorkedThread.HandleAllError(const IOCPData: PIOCPCommObj;
  const IOKey: DWORD);
var
...
procedure TAcceptExWorkedThread.HandleExcept(const IOCPData: PIOCPCommObj; const
  Str: string);
var
  PTRUser: PUserOBJ;
  StrExcept: string;
  ...
  begin
    ...
    case IOCPData.WorkType of
      SOCKET_READ:
        begin
          if PTRUser <> nil then
            StrExcept := Format('Read Buffer Error, UserIP:%s:%d',
              [PTRUser.pszIPAddr, PTRUser.Port]);
        end;
      SOCKET_SEND:
        begin
          //PSendData := PSendQueueNode(IOCPData);
          if PTRUser <> nil then
            StrExcept := Format('Send Buffer Error, UserIP:%s:%d',
              [PTRUser.pszIPAddr, PTRUser.Port]);
        end;
    end;
  end;
...
function TAcceptExWorkedThread.HandleRecvEvent(const IOCPData: PIOCPCommObj;
  const IOLength: DWORD): Boolean;
begin
...
function TAcceptExWorkedThread.HandleSendEvent(const IOCPData: PIOCPCommObj;
  const IOLength: DWORD): Boolean;
begin
...
begin
  ...
  try
    while True do
    begin
      fQueued := GetQueuedCompletionStatus(FIOCPPort, dwBytesTransferred,
        IOCPKey, PIOData, INFINITE);
      if (PIOData <> nil) then
      begin
    ...
    on E: Exception do
      HandleExcept(IOCPData,
        Format('TAcceptThread::GetQueuedCompletionStatus: %s', [E.Message]));
  end;
end;
...
procedure TIOCPAccepter.FreeAcceptEx(const IOCPData: PIOCPCommObj; const
...
procedure TIOCPAccepter.HandleAcceptFinished(const IOCPData: PIOCPCommObj;
  IOKey: DWORD);
var
...
begin
  ...
  try
    ...
    begin
      if g_pLogMgr.CheckLevel(5) then
        g_pLogMgr.Add(Format('超过每IP连接[%d]: %s', [g_pConfig.m_nMaxConnectOfIP,
          szRemoteIP]));
      bClose := True;
      Exit;
    end;
    ...
    begin
      HandleException(Format('TIOCPAccepter::SetSockOpt:%d',
        [WSAGetLastError()]), 5);
      bClose := True;
      Exit;
    end;
    ...
    if iRc = SOCKET_ERROR then
    begin
      HandleException(Format('TIOCPAccepter::CreateIoCompletionPort：%d',
        [GetLastError()]), 5);
      bClose := True;
      Exit;
    end;
    ...
    begin
      ...
      begin
        if g_pLogMgr.CheckLevel(7) then
          g_pLogMgr.Add(Format('用户登陆: %s', [szRemoteIP]));
      end;
    end
    else
      bClose := True;
...
procedure TIOCPAccepter.HandleException(const PTRMsgError: string; Level: Integer);
begin
  if g_pLogMgr.CheckLevel(Level) then
...
begin
  ...
  begin
    HandleException(Format('创建套接字失败，错误代码为:%d', [WSAGetLastError()]), 1);
    CloseHandle(FCompPort);
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('绑定到%d端口出错；请检查端口是否被占用', [Port]), 1);
    raise Exception.Create(Format('绑定到%d端口出错；请检查端口是否被占用', [Port]));
  end;
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('关联到完成端口错误，错误代码为:%d', [GetLastError()]), 1);
  end;
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('得到acceptEx地址错误，错误代码为：%d', [WSAGetLastError()]), 1);
  end;
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('得到GetAcceptExSockaddrs地址错误，错误代码为：%d', [WSAGetLastError()]), 1);
  end;
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('Listen错误，错误代码为：%d', [WSAGetLastError()]), 1);
  end;
...
procedure TIOCPAccepter.PostAcceptEx(ListenSocket: TSocket; LoginObj: PLoginObj
  = nil);
var
  NewSocket: TSocket;
...
begin
  ...
  if NewSocket = INVALID_SOCKET then
    HandleException(Format('新建立套接字错误，错误代码为:%d', [WSAGetLastError()]), 1);
  ...
  begin
    ...
    begin
      HandleException(Format('当前用户数太多:%d', [FAcceptExPool.InUseBlock]), 1);
      Exit;
    end;
  ...
  begin
    ...
    begin
      if WSAGetLastError() <> WSA_IO_PENDING then
        HandleException(Format('投递AccpetEx错误，错误代码为:%d', [WSAGetLastError()]), 5);
    end;
  end;
end;
...
function TIOCPReader.AddLoginUser(const IOCPData: PLoginObj; var ReaderOBJ:
  PRecvObj): Boolean;
var
  MemID: UINT;
  PTRUserOBJ: PUserOBJ;
...
constructor TIOCPReader.Create(MaxUser: Integer; SendSocket: TIOCPWriter;
  UserManager: TUserManager);
begin
  FMaxUser := MaxUser;
  FSendSocket := SendSocket;
...
function TIOCPReader.HandleReadData(const IOCPData: PIOCPCommObj; const IOLen:
  DWORD): BOOL;
var
  IOWorkOut: DWORD;
begin
...
function TIOCPReader.ReadDone(const IOCPData: PIOCPCommObj; const IOLen: DWORD):
  BOOL;
var
  PTRRecvOBJ: PRecvObj;
  iRecvLen: Integer;
...
function TIOCPWriter.AddLoginUser(IOCPData: PIOCPCommObj; var WriterOBJ:
  PIOCPCommObj): Boolean;
begin
  WriterOBJ := PIOCPCommObj(FSendQueue.GetSendQueue(IOCPData));
  Result := WriterOBJ <> nil;
end;

procedure TIOCPWriter.BroadUserMsg(wParam, lParam: Integer; OnBroadcastEvent:
  TOnBroadCastEvent);
begin
...
function TIOCPWriter.SendData(const WriterOBJ: PIOCPCommObj; const Buffer:
  PChar; const size: UINT): Boolean;
var
  iRc: Integer;
begin
  ...
  if fFreeMem then begin
    try
      ...
      on E: Exception do begin
        if g_pLogMgr.CheckLevel(5) then
        begin
          g_pLogMgr.Add(Format('TUserManager::DeleteUser %d: %s', [nCode,
            E.Message]));
        end;
      end;
    end;
