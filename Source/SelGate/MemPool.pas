...
type
  ...
  TFreeMemEvent = procedure(const MemID: UINT; const MemBuffer: PAnsiChar) of
    object;
  ...
  public
    ...
    property OnDestroyEvent: TFreeMemEvent read FOnDestroyEvent write
      FOnDestroyEvent;
...
begin
  ...
  begin
{$IFDEF SHDEBUG}
    SendMessage(hDebug, LB_ADDSTRING, 0,
      Integer(Format('Error MemoryAddr !!! MemID:%d LowAddr:%d HighAddr:%d InUseBlock:%d', [MemID, FMinPosition, FMaxPosition, FInUseBlock])));
{$ENDIF}
    Exit;
  end;
