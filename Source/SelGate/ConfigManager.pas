...
begin
  ...
  m_szTitle := ''; //'角色网关'; //原来是名称是： 角色网关，明显错误
  m_nShowLogLevel := 3;
...
begin
  ...
  m_nClientTimeOutTime := ReadInteger('Integer', 'ClientTimeOutTime2', m_nClientTimeOutTime);
  if m_nClientTimeOutTime < 10 * 1000 then
  begin
    m_nClientTimeOutTime := 10 * 1000;
    m_xIni.WriteInteger('Integer', 'ClientTimeOutTime2', m_nClientTimeOutTime);
  end;
  ...
  m_nGateCount := ReadInteger('SelGates_iocp', 'Count', 0);
  if m_nGateCount > 0 then
  begin
    for i := 1 to m_nGateCount do
    begin
      m_xGameGateList[i].sServerAdress := ReadString('SelGates_iocp', 'ServerAddr' + IntToStr(i), m_xGameGateList[i].sServerAdress);
      m_xGameGateList[i].nServerPort := ReadInteger('SelGates_iocp', 'ServerPort' + IntToStr(i), m_xGameGateList[i].nServerPort);
      m_xGameGateList[i].nGatePort := ReadInteger('SelGates_iocp', 'GatePort' + IntToStr(i), m_xGameGateList[i].nGatePort);
    end;
  ...
  begin
    ...
    m_xGameGateList[1].sServerAdress := ReadString('SelGate', 'ServerAddr', m_xGameGateList[1].sServerAdress);
    m_xGameGateList[1].nServerPort := ReadInteger('SelGate', 'ServerPort', m_xGameGateList[1].nServerPort);
    m_xGameGateList[1].nGatePort := ReadInteger('SelGate', 'GatePort', m_xGameGateList[1].nGatePort);
    ...
    case nType of
      0:
        begin
          ...
          WriteInteger('SelGates_iocp', 'Count', m_nGateCount);
          for i := 1 to m_nGateCount do
          begin
            WriteString('SelGates_iocp', 'ServerAddr' + IntToStr(i), m_xGameGateList[i].sServerAdress);
            WriteInteger('SelGates_iocp', 'ServerPort' + IntToStr(i), m_xGameGateList[i].nServerPort);
            WriteInteger('SelGates_iocp', 'GatePort' + IntToStr(i), m_xGameGateList[i].nGatePort);
          end;
        end;
      1:
        begin
          ...
          WriteInteger('Integer', 'ClientTimeOutTime2', m_nClientTimeOutTime);
