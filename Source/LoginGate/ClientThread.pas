...
begin
  if (LPNetEvent^.lNetworkEvents and FD_CLOSE) > 0 then
  begin
    FIsClose := True;
    Result := False;
    FErrorCode := 0;
...
begin
  if (LPNetEvent.lNetworkEvents and FD_CONNECT) > 0 then
  begin
    if LPNetEvent.iErrorCode[FD_CONNECT_BIT] <> 0 then
    begin
...
begin
  if (LPNetEvent.lNetworkEvents and FD_READ) > 0 then
  begin
    if LPNetEvent.iErrorCode[FD_READ_BIT] = 0 then
    begin
      Result := ReadData;
      if not Result then
      begin
...
begin
  ...
  try
    if InBufLen = 0 then
    begin //首先判断上次的数据是否发送完毕
      if uSend = 0 then
      begin //对付第一个FD_WRITE消息
        Result := True;
        Exit;
      end;
    end
    else
    begin //如果上次的数据没有发完,本次的数据和上次的数据合并发送
      Inc(uSend, InBufLen);
      if (uSend < SEND_BUFFER_LEN) then
      begin
        if (BufLen > 0) then
...
begin
  ...
  begin
    ...
    if not SafeSend(Buffer, BufLen) then
    begin
      SHSocket.FreeSocket(FSocket);
      DoActive(False);
{$IFDEF SHDEBUG}
...
begin
  if (LPNetEvent.lNetworkEvents and FD_WRITE) > 0 then
  begin
    if LPNetEvent.iErrorCode[FD_WRITE_BIT] = 0 then
    begin
...
begin
  ...
  with FRecvObj do
  begin
    if IOLen >= FRecvObj.RecvPos then
    begin
