...
begin
  ...
  begin
    ...
    Result := IOCPTypeDef.PostIOCPSend(IOCPData, ErrCode);
    if not Result then
    begin
      HandleExcept(IOCPData, Format('Error code: %d', [ErrCode]));
    end;
  end;
end;
...
var
  bClose: Boolean;
begin
  //HandleException('FreeAcceptEx close client', 5);
...
begin
  ...
  try
    ...
    try
      ...
      begin
        HandleException(Format('TIOCPAccepter::SetSockOpt:%d', [WSAGetLastError()]), 5);
        bClose := True;
        Exit;
      end;
      ...
      if iRc = SOCKET_ERROR then
      begin
        HandleException(Format('TIOCPAccepter::CreateIoCompletionPort：%d', [GetLastError()]), 5);
        bClose := True;
        Exit;
      end;
      ...
      begin
        Code := 30;
        if g_pLogMgr.CheckLevel(7) then
          g_pLogMgr.Add(Format('用户登陆失败: %s', [szRemoteIP]));
        bClose := True;
      end;
    ...
    g_pLogMgr.Add(Format('HandleAcceptFinished Error, Code: %d', [Code]));
  end;
end;
...
begin
  InterlockedExchange(Integer(IOCPData.Socket), Integer(INVALID_SOCKET));

  FRecvPool.LockMemPool;
  try
    FRecvPool.FreeMemory(IOCPData.MemIndex);
  finally
    FRecvPool.UnLockMemPool;
  end;
end;
...
begin
  ...
  begin
    // 2020-01-16
    if g_pLogMgr.CheckLevel(7) then
      g_pLogMgr.Add(Format('断开连接: %s', [UserOBJ.pszIPAddr]));
