...
begin
  ...
  if QuerySend then
  begin
    if not PostIOCPSend(ComOBJ, ErrCode) then
    begin
      Result := ADD_BUFFER_TO_QUEUE_SEND_ERROR;
      if g_pLogMgr.CheckLevel(4) then
        g_pLogMgr.Add('Send Buffer Failed, Error code: ' + IntToStr(ErrCode));
    end;
  end;
...
begin
  ...
  if QuerySend then
  begin
    Result := PostIOCPSend(ComOBJ, ErrCode);
  end;
end;
