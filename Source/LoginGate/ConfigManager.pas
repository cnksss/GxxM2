...
type
  ...
  TConfigMgr = class(TSyncObj) //Speed Lower then var record
    ...
    m_boCheckVersion: Boolean;
    m_sClientSoftVer: string;
...
begin
  ...
  m_nNomClientPacketSize := 700;
  m_nMaxConnectOfIP := 20;
  m_nCheckNewIDOfIP := 5;
  m_nClientTimeOutTime := 180 * 1000;
  m_nMaxClientPacketCount := 20;
end;
...
begin
  //String
  m_szTitle := ReadString('LoginGate', 'Title', m_szTitle);

  //Integer
  m_nShowLogLevel := ReadInteger('LoginGate', 'ShowLogLevel', m_nShowLogLevel);

  m_boCheckVersion := ReadBool('LoginGate', 'CheckClientSoft', m_boCheckVersion);
  m_sClientSoftVer := ReadString('LoginGate', 'ClientSoftVer', m_sClientSoftVer);

  m_nClientTimeOutTime := ReadInteger('Integer', 'ClientTimeOutTime3', m_nClientTimeOutTime);
  if m_nClientTimeOutTime < 10 * 1000 then
  begin
    m_nClientTimeOutTime := 10 * 1000;
    m_xIni.WriteInteger('Integer', 'ClientTimeOutTime3', m_nClientTimeOutTime);
  end;
  ...
  m_nNomClientPacketSize := ReadInteger('Integer', 'NomClientPacketSize2', m_nNomClientPacketSize);
  m_nMaxClientPacketCount := ReadInteger('Integer', 'MaxClientPacketCount2', m_nMaxClientPacketCount);
  ...
  m_nGateCount := ReadInteger('LoginGate', 'Count', 0);
  if m_nGateCount <= 0 then
  begin
    m_nGateCount := 1;
    m_xGameGateList[1].sServerAdress := ReadString('LoginGate', 'ServerAddr', m_xGameGateList[1].sServerAdress);
    m_xGameGateList[1].nServerPort := ReadInteger('LoginGate', 'ServerPort', m_xGameGateList[1].nServerPort);
    m_xGameGateList[1].nGatePort := ReadInteger('LoginGate', 'GatePort', m_xGameGateList[1].nGatePort);

    for I := 2 to MAX_SERVER_COUNT do
    begin
      m_xGameGateList[I].sServerAdress := m_xGameGateList[1].sServerAdress;
      m_xGameGateList[I].nServerPort := m_xGameGateList[1].nServerPort;
      m_xGameGateList[I].nGatePort := m_xGameGateList[1].nGatePort + I - 1;
    end;
  end
  else
  begin
    ...
    begin
      m_xGameGateList[i].sServerAdress := ReadString('LoginGate', 'ServerAddr' + IntToStr(i), m_xGameGateList[i].sServerAdress);
      m_xGameGateList[i].nServerPort := ReadInteger('LoginGate', 'ServerPort' + IntToStr(i), m_xGameGateList[i].nServerPort);
      m_xGameGateList[i].nGatePort := ReadInteger('LoginGate', 'GatePort' + IntToStr(i), m_xGameGateList[i].nGatePort);
    end;

    for I := m_nGateCount + 1 to MAX_SERVER_COUNT do
    begin
      m_xGameGateList[I].sServerAdress := m_xGameGateList[1].sServerAdress;
      m_xGameGateList[I].nServerPort := m_xGameGateList[1].nServerPort;
      m_xGameGateList[I].nGatePort := m_xGameGateList[1].nGatePort + I - 1;
...
begin
  ...
  begin
    case nType of
      0:
        begin
          WriteString('LoginGate', 'Title', m_szTitle);
          WriteInteger('LoginGate', 'ShowLogLevel', m_nShowLogLevel);

          WriteInteger('LoginGate', 'Count', m_nGateCount);
          for i := 1 to m_nGateCount do
          begin
            WriteString('LoginGate', 'ServerAddr' + IntToStr(i), m_xGameGateList[i].sServerAdress);
            WriteInteger('LoginGate', 'ServerPort' + IntToStr(i), m_xGameGateList[i].nServerPort);
            WriteInteger('LoginGate', 'GatePort' + IntToStr(i), m_xGameGateList[i].nGatePort);

            if I = 1 then
            begin
              WriteString('LoginGate', 'ServerAddr', m_xGameGateList[i].sServerAdress);
              WriteInteger('LoginGate', 'ServerPort', m_xGameGateList[i].nServerPort);
              WriteInteger('LoginGate', 'GatePort', m_xGameGateList[i].nGatePort);
            end
          end;

          WriteBool('LoginGate', 'CheckClientSoft', m_boCheckVersion);
          WriteString('LoginGate', 'ClientSoftVer', m_sClientSoftVer);
        end;
      1:
        begin
          //Integer
          ...
          WriteInteger('Integer', 'ClientTimeOutTime3', m_nClientTimeOutTime);
          WriteInteger('Integer', 'NomClientPacketSize2', m_nNomClientPacketSize);
          WriteInteger('Integer', 'MaxClientPacketCount2', m_nMaxClientPacketCount);
