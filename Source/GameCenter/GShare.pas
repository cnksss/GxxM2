...
interface
uses
  Windows, Messages, Classes, SysUtils, INIFiles, Common, GHeroDB, DataBackUp;
resourcestring
  m_sNoticeUrl = 'http://www.13m2.com';
  m_sGameName = 'BmM2';
  m_sCapTion = '引擎控制台';
  m_sGameList = 'BmM2|BmM2|127.0.0.1|7000|http://www.13m2.com';
  m_sVersion = '软件名称: BmM2反外挂数据引擎控制台';

const
  MAXRUNGATECOUNT = 8;

type
  TWindowHandle = UInt64; //THandle;
...
type
  TProgram = record
    ...
    MainFormHandle: TWindowHandle;  //THandle; //没有证据证实64位程序窗口句柄是64位的，保险起见
    nMainFormX: Integer;
    nMainFormY: Integer;
  end;
...
function RunProgram(var ProgramInfo: TProgram; sHandle: string; dwWaitTime: LongWord; btDouble: Byte = 0): LongWord;
function StopProgram(var ProgramInfo: TProgram; dwWaitTime: LongWord): Integer;
procedure SendProgramMsg(DesForm: UINT64; wIdent: Word; sSendMsg: string); //HZQ //THandle --> Uint64
//function CheckHeroDB(): Boolean;
//function CheckHeroDBTables(): Boolean;
//procedure SaveHeroDBConfig(const Name, Path: string);

var
  g_BackUpManager: TBackUpManager;
  g_boHeroDBOK: Boolean = False;
  //g_sNoticeUrl: string;
  g_sGameName: string;
...
//  g_nClientForm: Integer = -1;
  ...
  g_boUseSqliteDB: Boolean = False;
  g_sHeroDBName: string = 'HeroDB';
  g_sSqliteDBName: string = 'D:\MirServer\Mud2\DB\BmM2.db';
  ...
  g_sLocalIPaddr1: string = '127.0.0.2';

  g_sExtIPaddr: string = '192.168.0.1';
  g_sExtNetComIPaddr: string = '192.168.100.1';
  g_boDoubleLineMode: Boolean = False;                                                              // 双线一区模式
  g_nLimitOnlineUser: Integer = 10000;                                                              // 服务器最高上线人数
  g_boDynamicIPMode: Boolean = False;                                                               // 增加动态IP支持 piaoyun 2013-08-30

  g_sDBServer_ProgramFile: string = 'DBServer.exe';
  g_sDBServer_Directory: string = 'DBServer\';
  g_boDBServer_GetStart: Boolean = True;
  g_boDBServer_GetMinimize: Boolean = True;
  ...
  g_sDBServer_GateListFile: string = '!GateList.ini';
  ...
  g_boLoginServer_GetMinimize: Boolean = True;
  ...
  g_nLoginServer_ControlPort: Integer = 0;
  ...
  g_sLoginServer_IDLogDir: string = 'IDLogDir\';
  ...
  g_boLogServer_GetMinimize: Boolean = True;
  ...
  g_boM2Server_GetMinimize: Boolean = True;
  ...
  g_nM2Server_EditionId: Integer = 0;//版本ID
  g_nM2Server_AreaId: Integer = 0;//区服ID
  ...
  g_boSelGate_GetStart1: Boolean = False;
  g_boSelGate_GetMinimize: Boolean = True;
  ...
  g_boSelGate_GetMultiThread: Boolean = True;     //2019-09-28 15:45:08
  ...
  g_sRunGate_DirectoryEx: string = 'RunGate%d\';
  g_sRunGate_AddrTableFile: string = '!addrtable.txt';
  ...
  g_boRunGate_GetMinimize: Boolean = True;
  g_boRunGate_GetMultiThread: Boolean = False;

  g_sRunGate_ConfigFile: string = 'Config.ini';     //RunGate.ini
  ...
  g_nRunGate0_DBPort: Integer = 27201;
  g_nRunGate1_DBPort: Integer = 27301;
  g_nRunGate2_DBPort: Integer = 27401;
  g_nRunGate3_DBPort: Integer = 27501;
  g_nRunGate4_DBPort: Integer = 27601;
  g_nRunGate5_DBPort: Integer = 27701;
  g_nRunGate6_DBPort: Integer = 27801;
  g_nRunGate7_DBPort: Integer = 27901;

  g_nRunGateDBPort_MulThread: Integer = 27201;
  ...
  g_boEmbeddedWindow: Boolean = False;

  g_nAutoStartDelayTime: Integer = 60;
  g_boAutoStartServer: Boolean = False;
  g_nAutoStartTimeCount: Integer = 0;
  //g_boAutoStartBackUpServer: Boolean = False;
  //g_boTimerStart: Boolean = False; // 定时启动 piaoyun 2013-08-30
implementation
...
var
  I: Integer;
begin
  g_boEmbeddedWindow := g_IniConf.ReadBool('GameConf', 'EmbeddedWindow', g_boEmbeddedWindow);
  ...
  g_boUseSqliteDB := g_IniConf.ReadBool('GameConf', 'UseSqliteDB', g_boUseSqliteDB);
  g_sHeroDBName := g_IniConf.ReadString('GameConf', 'HeroDBName', g_sHeroDBName);
  g_sSqliteDBName := g_IniConf.ReadString('GameConf', 'SqliteDBName', g_sSqliteDBName);


  g_nDataSaveDBType := g_IniConf.ReadInteger('DataSaveDB', 'DataSaveDBType', g_nDataSaveDBType);
  g_sDataSaveDBServer := g_IniConf.ReadString('DataSaveDB', 'DataSaveDBServer', g_sDataSaveDBServer);
  g_wDataSaveDBPort := g_IniConf.ReadInteger('DataSaveDB', 'DataSaveDBPort', g_wDataSaveDBPort);
  g_sDataSaveDBUser := g_IniConf.ReadString('DataSaveDB', 'DataSaveDBUser', g_sDataSaveDBUser);
  g_sDataSaveDBPassword := g_IniConf.ReadString('DataSaveDB', 'DataSaveDBPassword', g_sDataSaveDBPassword);
  g_sDataSaveDataBase := g_IniConf.ReadString('DataSaveDB', 'DataSaveDataBase', g_sDataSaveDataBase);

  g_nM2Server_EditionId := g_IniConf.ReadInteger('GameConf', 'EditionId', g_nM2Server_EditionId);
  g_nM2Server_AreaId := g_IniConf.ReadInteger('GameConf', 'AreaId', g_nM2Server_AreaId);

  g_sGameName := g_IniConf.ReadString('GameConf', 'GameName', m_sGameName);
  g_sExtIPaddr := g_IniConf.ReadString('GameConf', 'ExtIPaddr', g_sExtIPaddr);
  g_sExtNetComIPaddr := g_IniConf.ReadString('GameConf', 'ExtNetComIPaddr', g_sExtNetComIPaddr);
  g_boDoubleLineMode := g_IniConf.ReadBool('GameConf', 'DoubleLineMode', g_boDoubleLineMode);
  // 增加动态IP支持 piaoyun 2013-08-30
  g_boDynamicIPMode := g_IniConf.ReadBool('GameConf', 'DynamicIPMode', g_boDynamicIPMode);

  g_nAutoStartDelayTime := g_IniConf.ReadInteger('GameConf', 'AutoStartDelayTime', g_nAutoStartDelayTime);
  g_boAutoStartServer := g_IniConf.ReadBool('GameConf', 'GetAutoStartServer', g_boAutoStartServer);
  //g_boAutoStartBackUpServer := g_IniConf.ReadBool('GameConf', 'GetAutoStartBackUpServer', g_boAutoStartBackUpServer);
  ...
  g_boDBServer_GetStart := g_IniConf.ReadBool('DBServer', 'GetStart', g_boDBServer_GetStart);
    ...
    g_boDBServer_GetMinimize := g_IniConf.ReadBool('DBServer', 'GetMinimize', g_boDBServer_GetMinimize);
  ...
  g_boM2Server_GetMinimize := g_IniConf.ReadBool('M2Server', 'GetMinimize', g_boM2Server_GetMinimize);
  ...
  g_boSelGate_GetStart1 := g_IniConf.ReadBool('SelGate', 'GetStart1', g_boSelGate_GetStart1);
  g_boSelGate_GetMinimize := g_IniConf.ReadBool('SelGate', 'GetMinimize', g_boSelGate_GetMinimize);
  g_boSelGate_GetMultiThread := g_IniConf.ReadBool('SelGate', 'GetMultiThread', g_boSelGate_GetMultiThread);      // 2019-09-28 15:44:24

  g_boRunGate_GetStart := g_IniConf.ReadBool('RunGate', 'GetStart', g_boRunGate_GetStart);
  g_boRunGate_GetMinimize := g_IniConf.ReadBool('RunGate', 'GetMinimize', g_boRunGate_GetMinimize);
  g_boRunGate_GetMultiThread := g_IniConf.ReadBool('RunGate', 'GetMultiThread', g_boRunGate_GetMultiThread);
  //g_boRunGate_GetMultiThread := False;                                                              //关闭多线程网关
  ...
  g_RunGateInfo[0].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort1', g_nRunGate0_DBPort);
  g_RunGateInfo[1].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort2', g_nRunGate1_DBPort);
  g_RunGateInfo[2].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort3', g_nRunGate2_DBPort);
  g_RunGateInfo[3].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort4', g_nRunGate3_DBPort);
  g_RunGateInfo[4].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort5', g_nRunGate4_DBPort);
  g_RunGateInfo[5].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort6', g_nRunGate5_DBPort);
  g_RunGateInfo[6].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort7', g_nRunGate6_DBPort);
  g_RunGateInfo[7].nDBPort := g_IniConf.ReadInteger('RunGate', 'DBPort8', g_nRunGate7_DBPort);

  g_nRunGateDBPort_MulThread := g_IniConf.ReadInteger('RunGate', 'DBPort_MulThread', g_nRunGateDBPort_MulThread);
...
//g_sDBServer_Config_GateAddr2 := g_sAllIPaddr;
  ...
  g_nLoginServer_ControlPort := g_IniConf.ReadInteger('LoginServer', 'ControlPort', g_nLoginServer_ControlPort);
  g_boLoginServer_GetStart := g_IniConf.ReadBool('LoginServer', 'GetStart', g_boLoginServer_GetStart);
  g_boLoginServer_GetMinimize := g_IniConf.ReadBool('LoginServer', 'GetMinimize', g_boLoginServer_GetMinimize);
  ...
  g_boLogServer_GetMinimize := g_IniConf.ReadBool('LogServer', 'GetMinimize', g_boLogServer_GetMinimize);
  g_nLogServer_Port := g_IniConf.ReadInteger('LogServer', 'Port', g_nLogServer_Port);
end;

function RunProgram(var ProgramInfo: TProgram; sHandle: string; dwWaitTime: LongWord; btDouble: Byte): LongWord;
var
...
begin
  ...
  sCommandLine := format('%s%s %s %d %d %d', [ProgramInfo.sDirectory, ProgramInfo.sProgramFile, sHandle, ProgramInfo.nMainFormX, ProgramInfo.nMainFormY, btDouble]);
...
procedure SendProgramMsg(DesForm: UINT64; wIdent: Word; sSendMsg: string);
var
...
initialization
  begin
    g_IniConf := Tinifile.Create(ExtractFilePath(ParamStr(0)) + g_sConfFile);
  end;
