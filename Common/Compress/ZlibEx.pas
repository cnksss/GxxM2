...
procedure CompressToUserBuf(const InBuf: Pointer; InBytes: Integer;
  const OutBuf: Pointer; out OutBytes: Integer);
  ...
  try
    try
      ...
      begin
        ...
        zstream.next_out := Pointer(NativeInt(outBuffer) + zstream.total_out);
...
begin
  ...
  try
    ...
    // 修复 chongchong 当解压出现异常OutBuffer 清空时，指针也清空
    outBuffer := nil;
    outSize := 0;
    raise;
  end;
...
begin
  ...
  try
    ...
    ZCompressCheck(deflateInit_(strm, Z_BEST_COMPRESSION, zlib_version, sizeof(strm)));
    try
      while ZCompressCheck(deflate(strm, Z_FINISH)) <> Z_STREAM_END do
      begin
        P := OutBuf;
        Inc(OutBytes, 256);
        //ReallocMem(OutBuf, OutBytes);
        strm.next_out := PChar(NativeInt(OutBuf) + (NativeInt(strm.next_out) - Integer(P)));
        strm.avail_out := 256;
      end;
    finally
      ZCompressCheck(deflateEnd(strm));
    end;
...
begin
  ...
  try
    ...
    strm.next_out := OutBuf;
    strm.avail_out := OutBytes;
    ZDecompressCheck(inflateInit_(strm, zlib_version, sizeof(strm)));
    try
      while ZDecompressCheck(inflate(strm, Z_FINISH)) <> Z_STREAM_END do
      begin
        P := OutBuf;
        Inc(OutBytes, BufInc);
        //ReallocMem(OutBuf, OutBytes);
        strm.next_out := Pointer(NativeInt(OutBuf) + (NativeInt(strm.next_out) - NativeInt(P)));
...
begin
  ...
  ZDecompressCheck(inflateInit_(strm, zlib_version, sizeof(strm)));
  try
    if ZDecompressCheck(inflate(strm, Z_FINISH)) <> Z_STREAM_END then
