...
type
  EIniFileExceptionEx = class(Exception);

  TCustomIniFileEx = class(TObject)
  private
    FFileName: string;
  public
  ...
  private
    FValueHash: TStringHashEx;
    FNameHash: TStringHashEx;
  ...
  TMemIniFileEx = class(TCustomIniFileEx)
  private
  ...
  TIniFileEx = class(TMemIniFileEx)
  public
    destructor Destroy; override;
  end;
...
constructor TCustomIniFileEx.Create(const FileName: string);
begin
  FFileName := FileName;
end;

function TCustomIniFileEx.SectionExists(const Section: string): Boolean;
var
  S: TStrings;
begin
...
function TCustomIniFileEx.ReadInteger(const Section, Ident: string;
...
procedure TCustomIniFileEx.WriteInteger(const Section, Ident: string; Value: Longint);
begin
  WriteString(Section, Ident, IntToStr(Value));
end;

function TCustomIniFileEx.ReadBool(const Section, Ident: string;
  Default: Boolean): Boolean;
begin
  Result := ReadInteger(Section, Ident, Ord(Default)) <> 0;
end;

function TCustomIniFileEx.ReadDate(const Section, Name: string; Default: TDateTime): TDateTime;
var
  DateStr: string;
begin
...
function TCustomIniFileEx.ReadDateTime(const Section, Name: string; Default: TDateTime): TDateTime;
var
  DateStr: string;
begin
  DateStr := ReadString(Section, Name, '');
...
function TCustomIniFileEx.ReadFloat(const Section, Name: string; Default: Double): Double;
var
  FloatStr: string;
begin
...
function TCustomIniFileEx.ReadTime(const Section, Name: string; Default: TDateTime): TDateTime;
var
  TimeStr: string;
begin
...
procedure TCustomIniFileEx.WriteDate(const Section, Name: string; Value: TDateTime);
begin
  WriteString(Section, Name, DateToStr(Value));
end;

procedure TCustomIniFileEx.WriteDateTime(const Section, Name: string; Value: TDateTime);
begin
  WriteString(Section, Name, DateTimeToStr(Value));
end;

procedure TCustomIniFileEx.WriteFloat(const Section, Name: string; Value: Double);
begin
  WriteString(Section, Name, FloatToStr(Value));
end;

procedure TCustomIniFileEx.WriteTime(const Section, Name: string; Value: TDateTime);
begin
  WriteString(Section, Name, TimeToStr(Value));
end;

procedure TCustomIniFileEx.WriteBool(const Section, Ident: string; Value: Boolean);
const
...
function TCustomIniFileEx.ValueExists(const Section, Ident: string): Boolean;
var
  S: TStrings;
begin
  S := TStringList.Create;
...
function TCustomIniFileEx.ReadBinaryStream(const Section, Name: string;
...
begin
  ...
  begin
    ...
    try
      ...
      HexToBin(PChar(Text), PChar(NativeInt(Stream.Memory) + Stream.Position), Length(Text) div 2);
      Stream.Position := Pos;
...
procedure TCustomIniFileEx.WriteBinaryStream(const Section, Name: string;
  Value: TStream);
var
  Text: string;
  Stream: TMemoryStream;
begin
  ...
  begin
    ...
    try
      ...
      BinToHex(PChar(NativeInt(Stream.Memory) + Stream.Position), PChar(Text),
...
begin
  ...
  if FNameHash = nil then
    FNameHash := TStringHashEx.Create
  else
    FNameHash.Clear;
...
begin
  ...
  if FValueHash = nil then
    FValueHash := TStringHashEx.Create
  else
    FValueHash.Clear;
  for I := 0 to Count - 1 do
    if not CaseSensitive then
...
begin
  ...
  FSections := THashedStringListEx.Create;
{$IFDEF LINUX}
...
destructor TMemIniFileEx.Destroy;
begin
...
function TMemIniFileEx.AddSection(const Section: string): TStrings;
var
  Index: Integer;
begin
  if FSections.Find(Section, Index) then
    Result := THashedStringListEx(FSections.Objects[Index])
  else
  begin
    Result := THashedStringListEx.Create;
    try
      THashedStringListEx(Result).CaseSensitive := CaseSensitive;
...
procedure TMemIniFileEx.EraseSection(const Section: string);
var
  I: Integer;
begin
  I := FSections.IndexOf(Section);
...
procedure TMemIniFileEx.GetStrings(List: TStrings);
var
  I, J: Integer;
  Strings: TStrings;
begin
...
procedure TMemIniFileEx.LoadValues;
var
  List: TStringList;
begin
...
procedure TMemIniFileEx.ReadSection(const Section: string;
  Strings: TStrings);
var
  I, J: Integer;
...
procedure TMemIniFileEx.ReadSections(Strings: TStrings);
begin
  Strings.Assign(FSections);
end;

procedure TMemIniFileEx.ReadSectionValues(const Section: string;
  Strings: TStrings);
var
  I: Integer;
begin
...
function TMemIniFileEx.ReadString(const Section, Ident,
...
procedure TMemIniFileEx.Rename(const FileName: string; Reload: Boolean);
begin
  FFileName := FileName;
  if Reload then
    LoadValues;
...
begin
  ...
  begin
    ...
    for I := 0 to FSections.Count - 1 do
      with THashedStringListEx(FSections.Objects[I]) do
      begin
        CaseSensitive := Value;
        Changed;
      end;
    THashedStringListEx(FSections).Changed;
  end;
end;

procedure TMemIniFileEx.SetStrings(List: TStrings);
var
  I, J: Integer;
  S: string;
  Strings: TStrings;
