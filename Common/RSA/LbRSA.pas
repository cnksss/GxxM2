...
type
  TLbRSA = class(TLbAsymmetricCipher)
    ...
    public {methods}
      ...
      function  DecryptString(const InString : AnsiString) : AnsiString; override;
      procedure EncryptFile(const InFile, OutFile : string); override;
      procedure EncryptStream(InStream, OutStream : TStream); override;
      function  EncryptString(const InString : AnsiString) : AnsiString; override;
      ...
      function EncryptBufferEx(const InBuf; InBufSize : Cardinal; var OutBuf) : Cardinal;
    public {properties}
...
procedure RSAEncryptStream(InStream, OutStream : TStream;
            Key : TLbRSAKey; Encrypt : Boolean);
function RSAEncryptString(const InString : AnsiString;
...
{ -------------------------------------------------------------------------- }
function TLbRSA.DecryptString(const InString : AnsiString) : AnsiString;
...
{ -------------------------------------------------------------------------- }
function TLbRSA.EncryptString(const InString : AnsiString) : AnsiString;
...
function TLbRSA.EncryptBufferEx(const InBuf; InBufSize: Cardinal;
  var OutBuf): Cardinal;
var
  InBlkCount  : Cardinal;
  InBlkSize, OutBlkSize: Cardinal;
  PlainBlockSize, CipherBlockSize : Integer;
  i : Integer;
  pInBlk, pOutBlk       : Pointer;
begin
  Result := 0;
  PlainBlockSize := cRSAPlainBlockSize[FPublicKey.KeySize];
  CipherBlockSize := cRSACipherBlockSize[FPublicKey.KeySize];

  InBlkSize := PlainBlockSize;
  OutBlkSize := CipherBlockSize;

  InBlkCount := InBufSize div InBlkSize;
  if (InBufSize mod InBlkSize) > 0 then
    Inc(InBlkCount);
  ...
  { process all except the last block }
  for i := 1 to (InBlkCount - 1) do
  begin
    EncryptRSAEx(FPublicKey, pInBlk, pOutBlk, InBlkSize);

    pInBlk := Pointer(Cardinal(pInBlk) + InBlkSize);
    pOutBlk := Pointer(Cardinal(pOutBlk) + OutBlkSize);

    Inc(Result, OutBlkSize);
  end;

  { process the last block }
  InBlkSize := InBufSize mod InBlkSize;
  if InBlkSize = 0 then
    InBlkSize := PlainBlockSize;

  i := EncryptRSAEx(FPublicKey, pInBlk, pOutBlk, InBlkSize);
  Inc(Result, i);
end;
