...
type
  ...
  public
    ...
    function SendBuf(var Buf; Count: Integer; IsFullInspection: Boolean = False): Integer;
    function SendStream(AStream: TStream): Boolean;
    function SendStreamThenDrop(AStream: TStream): Boolean;
    function SendText(const S: AnsiString; IsFullInspection: Boolean = False): Integer;
...
var
  ErrorCode: Integer;
  AName, AAddress, AService: AnsiString;
begin
  AName := Name;
  AAddress := Address;
  AService := Service;
        ...
        begin
          ...
          begin
            ...
            FLookupHandle := WSAAsyncGetHostByName(Handle, CM_LOOKUPCOMPLETE,
              PAnsiChar(AName), FGetHostData, MAXGETHOSTSTRUCT);
          ...
          begin
            FLookupState := lsLookupAddress;
            FAddr.sin_addr.S_addr := inet_addr(PAnsiChar(AAddress));
          end else
          begin
            ErrorCode := 1110;
        ...
        begin
          ...
          begin
            if FGetHostData = nil then
              FGetHostData := AllocMem(MAXGETHOSTSTRUCT);
            FLookupHandle := WSAAsyncGetHostByName(Handle, CM_LOOKUPCOMPLETE,
              PChar(AName), FGetHostData, MAXGETHOSTSTRUCT);
            CheckSocketResult(Ord(FLookupHandle = 0), 'WSAASyncGetHostByName');
            FService := Service;
            FPort := Port;
            FQueueSize := QueueSize;
        ...
        begin
          ...
          begin
            ...
            FLookupHandle := WSAASyncGetServByName(Handle, CM_LOOKUPCOMPLETE,
              PAnsiChar(AService), 'tcp' , FGetHostData, MAXGETHOSTSTRUCT);
...
begin
  AService := Service;
  ServEnt := getservbyname(PAnsiChar(AService), 'tcp');
  if ServEnt <> nil then
...
begin
  ...
  if Name <> '' then
    ...
    Result.sin_addr.s_addr := inet_addr(PAnsiChar(AAddress))
...
function TCustomWinSocket.SendText(const s: AnsiString; IsFullInspection: Boolean): Integer;
begin
  Result := SendBuf(Pointer(S)^, Length(S), IsFullInspection);
end;
...
function TCustomWinSocket.SendBuf(var Buf; Count: Integer; IsFullInspection: Boolean): Integer;
var
  ErrorCode: Integer;
  SendBuffer: PAnsiChar;
  //ErrCount_WSAEWOULDBLOCK: Integer;
  StartTick: Cardinal;
  nRet: Integer;
begin
  Lock;
  try
    ...
    if not IsFullInspection then
      Result := Send(FSocket, Buf, Count, 0)
    else
    begin
      // 修复客户端网络特差时，发送数据包不完整(丢包) chongchong 2013-12-23
      //ErrCount_WSAEWOULDBLOCK := 0;
      Result := 0;

      StartTick := GetTickCount;
      while Result < Count do
      begin
        SendBuffer := PAnsiChar(@Buf);
        Inc(SendBuffer, Result);
        nRet := Send(FSocket, SendBuffer^, Count - Result, 0);
        if nRet = SOCKET_ERROR then
        begin
          if WSAGetLastError = WSAEWOULDBLOCK then
          begin
            {
            if ErrCount_WSAEWOULDBLOCK < 6 then
              Inc(ErrCount_WSAEWOULDBLOCK);
            Sleep(100 * ErrCount_WSAEWOULDBLOCK)
            }
            Sleep(10);
          end
          else
        ...
        if GetTickCount - StartTick >= 60 * 1000 then Break;
...
begin
  ...
  begin
    ...
    try
      for I := 0 to FActiveThreads.Count - 1 do
      begin
        TServerClientThread(FActiveThreads[I]).FKeepInCache := I < Start;
      end;
    finally
      FListLock.Leave;
    end;
