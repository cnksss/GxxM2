...
// https://fossil.2of4.net/_synopse/finfo?name=SynSQLite3Static.pas&m=581b0192eec01920
...
type
  ...
  PMyAnsiString = PAnsiChar;
  TMysize_t = NativeUInt;
  TMyAnsiChar = AnsiChar;
...
type
  ...
  GPtr          = PMyAnsiString;
  PPByte        = ^PByte;
  my_pchar      = PMyAnsiString;
  my_ppchar     = ^my_pchar;
  my_ulong      = LongWord;
  my_long       = LongInt;
  my_size_t     = TMysize_t;
  Pmy_ulong     = ^my_ulong;
  PPmy_ulong    = ^Pmy_ulong;
  Pmy_long      = ^my_long;
...
type
  ...
  NET0510 = record
    ...
    last_error:       array[0 .. MYSQL_ERRMSG_SIZE_0500 - 1] of TMyAnsiChar;
    // Client library sqlstate buffer. Set along with the error message.
    sqlstate:         array[0 .. SQLSTATE_LEN] of TMyAnsiChar;
    extension:        Pointer;
  end;
  PNET0506 = ^NET0506;
  NET0506 = record
  ...
  NET0410 = record
    ...
    // Pointer to query object in query cache, do not equal NULL (0) for
    // queries in cache that have not stored its results yet
    last_error:       array[0 .. MYSQL_ERRMSG_SIZE_0300 - 1] of TMyAnsiChar;
    sqlstate:         array[0 .. SQLSTATE_LEN] of TMyAnsiChar;
    last_errno:       Cardinal;
    error:            Byte;
    query_cache_query:GPtr;
...
{>= 4.1 end}
  ...
  MYSQL0570 = record
    ...
    scramble:             array[0..SCRAMBLE_LENGTH] of TMyAnsiChar;
  ...
  MYSQL0400 = record
    ...
    free_me,                            // If free in mysql_close
    reconnect:            my_bool;      // set to 1 if automatic reconnect
    scramble_buff:        array[0..SCRAMBLE_LENGTH_323] of TMyAnsiChar;
    // Set if this is the original connection, not a master or a slave we have
    // added though mysql_rpl_probe() or mysql_set_master()/ mysql_add_slave()
    rpl_pivot:            my_bool;
    // Pointers to the master, and the next slave connections, points to
...
//    fetch_result: TMYSQL_BIND_fetch_result;
  ...
  MYSQL_STMT0570 = record
    ...
    last_error: array [0 .. MYSQL_ERRMSG_SIZE_0500 - 1] of TMyAnsiChar; // error message
    sqlstate: array [0 .. SQLSTATE_LEN] of TMyAnsiChar;
  ...
  MYSQL_STMT0411 = record
    ...
    field_count: Cardinal;         // fields count
    state: PREP_STMT_STATE0411;    // statement state
    last_error: array [0 .. MYSQL_ERRMSG_SIZE_0300 - 1] of TMyAnsiChar; // error message
    sqlstate: array [0 .. SQLSTATE_LEN] of TMyAnsiChar;
    // Types of input parameters should be sent to server
    send_types_to_server: my_bool; // Types sent to server
    bind_param_done: my_bool;      // input buffers were supplied
