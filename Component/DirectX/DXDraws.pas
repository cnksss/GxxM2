...
type
  ...
  TDXDrawGetSurfaceEvent = procedure(Sender: TCustomDXDraw; var Surface: TDirectDrawSurface) of object;
  ...
  private
    ...
    FOnGetSurface: TDXDrawGetSurfaceEvent;
  ...
  public
    ...
    property OnGetSurface: TDXDrawGetSurfaceEvent read FOnGetSurface write FOnGetSurface;
...
function RGB32(C: Word): TColor; overload;
begin
  Result := RGB(C and $F800 shr 8, C and $07E0 shr 3, C and $001F shl 3);
end;

function RGB32(Col: TColor; BitCount: Byte): LongInt; overload;
begin
  if BitCount = 16 then
    Result := Word((GetRValue(Col) and $F8 shl 8) or (GetGValue(Col) and $FC shl 3) or (GetBValue(Col) and $F8 shr 3))
  else
    if (BitCount = 24) or (BitCount = 32) then
    Result := GetRValue(Col) shl 16 or GetGValue(Col) shl 8 or GetBValue(Col)
  else Result := Col;
end;

function BGR32(Col: TColor): LongInt;
begin
  Result := GetBValue(Col) shl 16 or GetGValue(Col) shl 8 or GetRValue(Col);
end;
...
begin
  ...
  DBltEx.dwFillColor := RGB32(DevColor, BitCount); //ColorToRGB(DevColor);
...
begin
  DBltEx.dwSize := SizeOf(DBltEx);
  DBltEx.dwFillColor := RGB32(DevColor, BitCount);
  DestRect := Rect;
    ...
    begin
      try
        case ddsd.ddpfPixelFormat.dwRGBBitCount of
          ...
          16: Result := RGB32(PWord(Integer(ddsd.lpSurface) + Y * ddsd.lPitch + X * 2)^);
    ...
    begin
      try
        case ddsd.ddpfPixelFormat.dwRGBBitCount of
          ...
          16: PWord(Integer(ddsd.lpSurface) + Y * ddsd.lPitch + X * 2)^ := RGB16(Value);
          24: with PRGB(Integer(ddsd.lpSurface) + Y * ddsd.lPitch + X * 3)^ do
            begin
              R := Byte(Value);
              G := Byte(Value shr 8);
          ...
          32: PInteger(Integer(ddsd.lpSurface) + Y * ddsd.lPitch + X * 4)^ := RGB32(Value, BitCount); //Value;
        end;
      finally
        UnLock;
      end;
...
begin
  ...
  FDXDraw.FSurface := nil;
  if Assigned(FDXDraw.OnGetSurface) then
    FDXDraw.OnGetSurface(FDXDraw, FDXDraw.FSurface);
  if FDXDraw.FSurface = nil then
