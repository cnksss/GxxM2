...
type
  ...
  TLightUTDist=array[0..255, 0..255] of Integer;
...
var
  LightUTDist: TLightUTDist;

implementation

uses {$IFDEF PNG_GRAPHICS}pngimage,{$ENDIF} jpeg;
...
begin
  ...
  case ABitCount of
    1: if not ((PixelFormat.RBitMask = $FF0000) and (PixelFormat.GBitMask = $00FF00) and (PixelFormat.BBitMask = $0000FF)) then
        raise EInvalidGraphicOperation.Create('PixelFormat in invalid');
    ...
    raise EInvalidGraphicOperation.CreateFmt('Bitcount in invalid (%d)', [ABitCount]);
  end;
  ...
  begin
    ...
    begin
      ...
      if FHandle = 0 then
        raise EOutOfResources.CreateFmt('%s cannot be made', ['DIB']);
...
begin
  ...
  if i <> 4 then
    raise EInvalidGraphic.Create('DIB is invalid');
...
begin
  ...
  if (Y < 0) or (Y >= FHeight) then
    raise EInvalidGraphicOperation.CreateFmt('Index of the scanning line exceeded the range. (%d)', [Y]);
...
begin
  for i := 0 to 255 do // Build Lightning LUT
    for j := 0 to 255 do
      LightUTDist[i, j] := ROUND(Sqrt(Sqr(i * 10) + Sqr(j * 10)));
end;
