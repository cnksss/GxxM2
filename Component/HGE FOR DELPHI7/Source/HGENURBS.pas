...
begin
  ...
  for i := 0 to CPCount - 1 do
    begin
      ...
      Value := TPoint2Add(Value , ControlPoints[i]);
      Value := TPoint2Multiply(Value,N) ;
...
begin
  if not FittingCurveReady then SetFittingCurve;
  Result := 0;
  for i := Index1 to Index2 - 1 do  begin

    Result := Result + Length2(TPoint2Subtract(Point2(FittingCurve[i].x, FittingCurve[i].y),Point2(FittingCurve[i+1].x, FittingCurve[i+1].y))
                               );
  end;
end;
  ...
  case FittingCurveType of
    fcConstantParameter:
      begin
        ...
        Result := Angle2(TPoint2Subtract(p2 , p1));
      end;
    fcConstantSpeed:
      begin
        Index := Trunc(Parameter*Segments);
        if Index >= Segments - 1 then Index := Segments - 2;
        Result := Angle2(TPoint2Subtract(FittingCurve[Index+1] ,FittingCurve[Index]));
      end;
  end;
end;
