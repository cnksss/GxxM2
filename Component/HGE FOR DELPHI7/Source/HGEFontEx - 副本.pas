...
interface
uses
  Windows, Classes, SysUtils, Graphics, HGE, HashList {, IGDIPlus};
const
...
type
  ...
  public
    ...
    procedure TextRect(const X, Y: Integer; SrcRect: TRect; ImageInfos: TImageInfos; Color: TColor = clWhite; BlendMode: Integer = Blend_Default; Alpha: Byte = 255); overload;
    ...
    function Add(const Text: string): pTImageRect;
...
begin
  ...
  FProcIdx := 0;
  FList := THashList.Create(ATextCount);                                                            //ATextCount
...
begin
  ...
  for I := 0 to Length(FImageRect) - 1 do
  begin
    if not FImageRect[I].Active then
    begin
  ...
  begin
    ...
    begin
     ...
     // ImageRect := Images[nIdx];
      ...
      begin
        if GetTickCount - ImageRect.Time > m_dwFreeMemCheckTime then
        begin
      ...
      Inc(nIdx);
      if (GetTickCount - dwTimeTick) > 10 then
      begin
...
begin
  ...
  Windows.GetTextExtentPoint32(HHDC, '0', 1, TextSize);
  ...
  Windows.GetTextExtentPoint32(HHDC, '一', 2, TextSize);
...
begin
  sText := Text;
  if Length(Text) = Length(sText) then
  begin
    Result := FFontHeight;
  end
  else
...
function THGEFont.Add(const Text: string): pTImageRect;
var
...
begin
  Result := nil;
  if (not FInitialized) or (not GameCanvas.Active) then
  begin
  ...
  //位图信息头
    ...
    begin
      ...
      //try
      try
        if FTexture.Lock(SrcRect, Bits, Pitch, False) then
        begin
...
procedure THGEFont.TextRect(const X, Y: Integer; SrcRect: TRect; ImageInfos: TImageInfos; Color: TColor; BlendMode: Integer; Alpha: Byte);
var
  I, II: Integer;
...
begin
  ...
  begin
    ...
    for I := 0 to Length(ImageInfos) - 1 do
    begin
      if nHeight <= 0 then break;
      if ImageInfos[I].Height <= 0 then
      begin
        if nOffsetY >= TextHeight('0') then
        begin
      ...
      if (nOffsetY > 0) and (nOffsetY >= ImageInfos[I].Height) then
      begin
      ...
      for II := 0 to Length(ImageInfos[I].ImageIndexs) - 1 do
      begin
        if nWidth <= 0 then break;
        ImageRect := Images[ImageInfos[I].ImageIndexs[II]];
        if (ImageRect <> nil) and (ImageRect.Rect.Right > ImageRect.Rect.Left) then
        begin
          if (nOffsetX > 0) then
          begin
            if (nOffsetX >= ImageRect.Rect.Right - ImageRect.Rect.Left) then
            begin
...
begin
  ...
  begin
    ...
    for I := 0 to Length(ImageIndexs) - 1 do
    begin
      if nWidth <= 0 then break;
 ...
 // try
  ...
  begin
    ...
    if (CompareText(HGEFont.Font.Name, AFontName) = 0) and
      ...
      (HGEFont.Font.Style = AFontStyles) then
    begin
      Result := HGEFont;
      boFind := True;
      break;
