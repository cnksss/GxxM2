...
type
  ...
  TThreadTimerEventTime = (tetPre, tetPost);
  ...
  private
    ...
    FTimer: TTimer;
    FEventTime: TThreadTimerEventTime;
    {$IFDEF MSWINDOWS}
  ...
  published
    property EventTime: TThreadTimerEventTime read FEventTime write FEventTime default tetPre;
...
constructor TThreadTimerThread.Create(Timer: TThreadTimer; Enabled: Boolean);
begin
  FOwner := Timer;
...
procedure TThreadTimerThread.HandleException;
begin
  if not (FException is EAbort) then
    Application.HandleException(Self);
end;

procedure TThreadTimerThread.SetPaused(const Value: Boolean);
begin
...
destructor TThreadTimerThread.Destroy;
begin
  inherited Destroy;
...
var
  EventTime: TThreadTimerEventTime;
...
function TThreadTimerThread.GetPaused: Boolean;
begin
  FPauseSection.Acquire;
  Result := FPaused;
  FPauseSection.Release;
...
begin
  ...
  begin
    ...
    (FTimerThread as TThreadTimerThread).Paused := False;
    FTimerThread.Free;
  end;
  FTimer.Free;
  inherited Destroy;
...
procedure TThreadTimer.DoTimer(Sender: TObject);
begin
  Timer;
end;
  ...
  begin
    ...
    begin
      ...
      if not Assigned(FTimerThread) then
        FTimerThread := TThreadTimerThread.Create(Self, False);

      TThreadTimerThread(FTimerThread).Paused := True;
      TThreadTimerThread(FTimerThread).FCurrentDuration := 0;
      TThreadTimerThread(FTimerThread).FInterval := FInterval;

      {$IFDEF MSWINDOWS}
      FTimerThread.Priority := FThreadPriority;
      {$ENDIF MSWINDOWS}

      TThreadTimerThread(FTimerThread).Paused := False;
    end
    else
    begin
...
begin
  ...
  begin
    if FInTimerEvent then
      raise Exception.CreateResFmt(@RsCannotChangeInTimerEvent, ['TThreadTimer.Threaded']); // do not localize
    FThreaded := Value;
    UpdateTimer;
  end;
end;
...
begin
  if FTimerThread <> nil then
  begin
    with TThreadTimerThread(FTimerThread) do
    begin
      if Suspended or Terminated then
        Method
      else
        TThreadTimerThread(FTimerThread).Synchronize(Method);
    end;
  end
  else
    Method;
