...
//#define NULL ((void *)0)
  ...
  try
    GetMem(Result, SizeOf(TRS) * 1); //  rs = (RS *)calloc(1,sizeof(RS));
...
begin
  ...
  while rs <> nil do begin
    ...
    and (rs.mm = symsize) and (rs.gfpoly = gfpoly) and (rs.fcr = fcr) then begin
       Result := rs;
       Exit;
    end;
    rs := rs.next;
...
begin
  ...
  for i := 0 to rs.nn - rs.nroots - rs.pad - 1 do begin
    feedback := index_of[data_a[i] xor parity_a[0]];
    if feedback <> rs.nn then begin  {* feedback term is non-zero *}
      {$IFDEF UNNORMALIZED}
      ...
      {$ENDIF}
      for j := 1 to rs.nroots - 1 do begin
          parity_a[j] := parity_a[j] xor alpha_to[modnn(rs, feedback + genpoly[rs.nroots - j])];
      end;
    end;
    {* Shift *}
     MoveMemory(@parity_a[0], @parity_a[1], SizeOf(data_t) * (rs.nroots - 1));
    if (feedback <> rs.nn) then begin
