...
begin
  Result := nil;
	if (QRinput_check(mode, size, data) <> 0) then begin
  //		errno = EINVAL;
    Exit;
	end;
...
begin
	if input <> nil then begin
		list := input.head;
		while (list <> nil) { and (Integer(list.mode) >= -1) and (Integer(list.mode) <= 7)} do begin
		  	next := list.next;
...
var
  ...
  ret: Integer;
begin            
	if input.mqr <> 0 then begin
		if (QRinput_createBitStream(input) < 0) then begin
			Result := nil;
      Exit;
		end;
	end else begin
		if (input.fnc1 <> 0) then begin
			if (QRinput_insertFNC1Header(input) < 0) then begin
				Result := nil;
        Exit;
			end;
		end;

		if (QRinput_convertData(input) < 0) then begin
			Result := nil;
      Exit;
		end;
	end;
