...
uses
  rscode, qrinput, qrspec, mqrspec, qrmask, mmask, split;
...
procedure RSblock_initBlock(block: PRSblock; dl: Integer; data: PByte; el: Integer; ecc: PByte; rs: PRS);
begin
	block.dataLength := dl;
	block.data := data;
	block.eccLength := el;
...
function RSblock_init(blocks: PRSblock; const spec: TArrayInt5; data, ecc: PByte): Integer;
var
  i, el, dl: Integer;
  block: PRSblock;
  dp, ep: PByte;
...
begin
	...
	dp := data;
	ep := ecc;
	for i := 0 to QRspec_rsBlockNum1(spec) - 1 do begin
		RSblock_initBlock(block, dl, dp, el, ep, rs); //HZQ 执行到此处会造成内存越界异常崩溃
		Inc(dp, dl);
		Inc(ep, el);
		Inc(block);
	end;
  
	if QRspec_rsBlockNum2(spec) = 0 then begin
    Result := 0;
    Exit;
  end;
	...
	for i := 0 to QRspec_rsBlockNum2(spec) - 1 do begin
...
var
  spec:TArrayInt5; //array[0..5-1] of Integer;
  ret: Integer;
begin
  try
  ...
  begin
	...
	QRspec_getEccSpec(input.version, input.level, PInteger(@spec[0])); 
  ...
  try
    ////HZQ 20230606 raw->rsblock = (RSblock *)calloc(raw->blocks, sizeof(RSblock)); 修复二维码引起的内存崩溃
    GetMem(Result.rsblock, SizeOf(TRSblock) * Result.blocks);
    ZeroMemory(Result.rsblock, SizeOf(TRSblock) * Result.blocks);
  except
    QRraw_free(Result);
		Result := nil;
    Exit;
	...
	ret := RSblock_init(Result.rsblock, spec, Result.datacode, Result.ecccode); //HZQ 定位到此函数有问题，会内存越界
...
//		errno := EINVAL;
	...
	if (hint <> QR_MODE_8) and (hint <> QR_MODE_KANJI) then begin
//		errno := EINVAL;
		Result := nil;
    Exit;
	end;
