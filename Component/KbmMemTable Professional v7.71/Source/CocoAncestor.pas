...
{$ENDIF}
  ...
  TLiteralList = TDictionary<string,integer>;

{$IFNDEF FPC}
  TLiteralEqualityComparer = class(TEqualityComparer<string>)
  public
...
{$ENDIF}
  ...
  public
    ...
    property Literals: TLiteralList read fLiterals write fLiterals;
  end;
...
function CreateLiterals(const ACaseSensitive:boolean; const Names: array of String; const Ids: array of Integer): TLiteralList;
...
var I: Integer;
begin
  Result := TLiteralList.Create{$IFNDEF FPC}(TLiteralEqualityComparer.Create){$ENDIF};
  for I := Low(Names) to High(Names) do
    Result.Add(Names[I], Ids[I]);
{$IFNDEF NEXTGEN}
...
{ TLiteralEqualityComparer }

{$IFNDEF FPC}
function TLiteralEqualityComparer.Equals(const Left, Right:string):boolean;
begin
     Result:=SameText(Left,Right);
end;

function TLiteralEqualityComparer.GetHashCode(const Value:string):integer;
var
   s:string;
begin
     s:=UpperCase(Value);
...
begin
  if (pos<0)or(pos>=SourceLen) then
     ...
     ch:=0; // Remove warnign for windows compiler. Other compilers issue warning about ch not initialized if this line is removed.
     while True do
     begin
...
begin
  ...
  begin
       if fLiterals.TryGetValue(str,I) then
	   	  Sym:=I;
  end;
end;
...
begin
  ...
  for I := 0 to C-1 do
  begin
    aSymSets[I] := TBits.Create;
    aSymSets[I].Size:=mI+1;
{$IFNDEF NEXTGEN}
...
begin
  ...
  if (fLiterals<>nil) then
  begin
       if fLiterals.TryGetValue(LexNames[1],sym) then
          if sym=-Id then
...
{$IFNDEF NEXTGEN}
   _GlobalCleanUps := TObjectList.Create;
   _GlobalCleanUps.OwnsObjects:=true;
