...
const
     KbmMemTableDesignerVersion='Designer - TkbmMemTable v.'+KBMMEMTABLE_VERSION+' '+KBMMEMTABLE_EDITION+' Edition';
...
begin
     ...
     try
      ...
      {$ELSE}
        ...
        begin
             Item:=LTV_Alias.Items.Add;
             Item.Caption:=string(DB_Description.szName);
             Item.SubItems.Add(string(DB_Description.szPhyName));
        end;
       {$ENDIF}
...
function CreateUniqueName(DataSet:TDataSet; const FieldName:string; FieldClass:TFIeldClass; Component:TComponent):string;
   ...
   begin
        ...
        with DataSet.Owner do
             for i:=0 to ComponentCount - 1 do
                 if (Component<>Components[i]) and (CompareText(AName,Components[i].Name)=0) then
                    exit;
        Result:=true;
   end;

  function GenerateName(DataSet:TDataSet; FieldName:string; FieldClass:TFIeldClass; Number:integer):string;
     ...
     begin
          ...
          begin
               if CharInSet(FieldName[i], ['A'..'Z', 'a'..'z', '_', '0'..'9']) then
//               if FieldName[i] in ['A'..'Z', 'a'..'z', '_', '0'..'9'] then
                  inc(i)
               else if CharInSet(FieldName[i], LeadBytes) then
  ...
  begin
       CrunchFieldName;
       if (FieldName = '') or (CharInSet(FieldName[1], ['0'..'9'])) then
       begin
...
begin
     ...
     begin
          ...
          begin
               ...
               try
                  Field.Origin:='';
                  Field.Name := CreateUniqueName(MemTable, Items[i].Name, TFieldClass(Items[i].ClassType), nil);
               except
                  Field.Free;
                  raise;
               end;
