...
uses System.Variants, Winapi.Windows, System.SysUtils, System.Classes, Data.DB,
  Data.DBCommon, Data.DBCommonTypes, BDE, Bde.SMINTF, System.Generics.Collections;

const

{$HPPEMIT ''}
{$HPPEMIT '/* automatically link to dblogdlg.obj so that the login dialog is automatically shown */'}
{$HPPEMIT '#pragma link "vcl.dblogdlg"'}
...
{$HPPEMIT '#endif'}
{$HPPEMIT '/* The BDE is supported on Win32 only */'}
{$HPPEMIT '#if !defined(WIN32) || defined(_WIN64)'}
{$HPPEMIT '#pragma message("BDERTL is only supported for WIN32")'}
{$HPPEMIT '#endif'}
{$HPPEMIT ''}
...
{ TBDEDataSet }
  ...
  protected
    ...
    function CreateLookupFilter(Fields: TList<TField>; const Values: Variant;
      Options: TLocateOptions; Priority: Integer): HDBIFilter; overload;
    function CreateLookupFilter(Fields: TList; const Values: Variant;
      Options: TLocateOptions; Priority: Integer): HDBIFilter; overload; deprecated 'Use overloaded method instead';
...
uses Data.DBConsts, Bde.Bdeconst, Winapi.ActiveX, System.Types;
...
begin
  if ((Param.DataType in [ftString, ftFixedChar]) and (Length(Param.AsAnsiString) > 255)) or
     (Param.DataType in [ftBlob..ftTypedBinary,ftOraBlob,ftOraClob]) then
    Result := SizeOf(BlobParamDesc)
  else if (Param.DataType in WideDataTypes) then
    Result := Length(Param.AsAnsiString)
  else
    Result := Param.GetDataSize;
end;
...
procedure GetParamData(Param: TParam; Buffer: Pointer; const DrvLocale: TLocale);
  ...
  // a string instead of an AnsiString or an array of byte.
...
var
  ...
  NativeBytes: TArray<Byte>;
begin
  if Param.DataType in [ftString, ftFixedChar, ftMemo] + WideDataTypes then
  begin
    AnsiNativeStr := VarToAnsiStr(Param.Value);
    if (Length(AnsiNativeStr) > 255) or
       (Param.DataType = ftMemo) or
       (Param.DataType = ftWideMemo) then
    begin
      ...
      Param.NativeStr := MakeNativeStr(Pointer(AnsiNativeStr), Length(AnsiNativeStr));
      BlobParamDesc(Buffer^).pBlobBuffer := Pointer(Param.NativeStr);
      BlobParamDesc(Buffer^).ulBlobLen := Length(AnsiNativeStr);
    end
    else
    begin
      if (DrvLocale <> nil) then
  ...
  else if (Param.DataType in [ftBlob..ftTypedBinary,ftOraBlob,ftOraClob]) then
  begin
    NativeBytes := Param.AsBytes;
    // Make sure the string data pointed to by pBlobBuffer is reference counted.
    Param.NativeStr := MakeNativeStr(Pointer(NativeBytes), Length(NativeBytes));
    BlobParamDesc(Buffer^).ulBlobLen := Length(NativeBytes);
    BlobParamDesc(Buffer^).pBlobBuffer := Pointer(Param.NativeStr);
  end
  else
    Param.GetData(Buffer);
...
begin
  ...
  try
    for I := 0 to Params.Count - 1 do
    begin
      FieldDescs[I].iFldType := FldTypeMap[Params[I].DataType];
      if FieldDescs[I].iFldType in [fldBlob, fldZString] then
        FieldDescs[I].iSubType := FldSubTypeMap[Params[I].DataType]
      else if FieldDescs[I].iFldType = fldUNKNOWN then
        DatabaseErrorFmt(SNoParameterValue, [Params[I].Name], Sender);
      FieldDescs[I].iFldNum := I + 1;
      FieldDescs[I].iLen := GetParamDataSize(Params[I]);
      GetParamData(Params[I], CurPtr, DrvLocale);
      FieldDescs[I].iOffset := CurPtr - BufStart;
      if Params[I].IsNull then
        FieldDescs[I].iNullOffset := NullPtr - BufStart
      else if FieldDescs[I].iFldType = fldZString then
      begin
        if Params[I].DataType in WideDataTypes then
          FieldDescs[I].iUnits1 := Length(Params[I].AsAnsiString)
        else
          FieldDescs[I].iUnits1 := Params[I].GetDataSize - 1 {Do not include null terminator}
      end
      else if FieldDescs[I].iFldType = fldBYTES then
        FieldDescs[I].iUnits1 := Params[I].GetDataSize
      else if FieldDescs[I].iFldType = fldVARBYTES then
        FieldDescs[I].iUnits1 := Params[I].GetDataSize - 2
      else if FieldDescs[I].iFldType = fldBlob then
        FieldDescs[I].iSubType := FldSubTypeMap[Params[I].DataType];
      Inc(CurPtr, FieldDescs[I].iLen);
    end;
...
procedure TimerCallBack(hWnd: HWND; Message: UINT; TimerID: UINT_PTR;
  SysTime: DWORD); stdcall;
begin
  FreeTimer;
end;
...
function AnsiToNative(Locale: TLocale; const UStr: string;
  NativeStr: PAnsiChar; MaxLen: Integer): PAnsiChar; overload;
begin
  Result := AnsiToNative(Locale, AnsiString(UStr), NativeStr, MaxLen);
end;

procedure NativeToAnsi(Locale: TLocale; NativeStr: PAnsiChar; var UStr: string); overload;
var
  ...
  try
    ...
    try
      for I := 0 to SList.Count - 1 do
        List.Add(TSession(SList[I]).SessionName);
    finally
      FSessions.UnlockList;
    end;
  finally
...
begin
  ...
  if (Database.FRefCount = 0) and not Database.KeepConnection then
    if not Database.Temporary then Database.Close else
      if not (csDestroying in ComponentState) then Database.Free;
end;
...
begin
  Result := cbrYES;
  try
    if PCBDBLogin(CBInfo)^.hDB = nil then
    begin
      if not FBDEOwnsLoginCbDb then
      begin
        Database := OpenDatabase(string(PCBDBLogin(CBInfo)^.szDbName));
        if not Database.HandleShared then
        begin
          PCBDBLogin(CBInfo)^.hDb := Database.Handle;
          PCBDBLogin(CBInfo)^.bCallbackToClose := True;
        end
        else
        begin
      ...
      begin
        AliasParams := TStringList.Create;
        try
          GetAliasParams(string(PCBDBLogin(CBInfo)^.szDbName), AliasParams);
        ...
        if Assigned(LoginDialogExProc) then
          if LoginDialogExProc(string(PCBDBLogin(CBInfo)^.szDbName), UserName, Password, True) then
          begin
            AnsiToNative(Locale, Password, PCBDBLogin(CBInfo)^.szPassword, SizeOf(PCBDBLogin(CBInfo)^.szPassword) - 1);
            PCBDBLogin(CBInfo)^.bCallbackToClose := False;
          end
          else
            Result := cbrAbort;
      end
    ...
    begin
      Database := FindDatabase(string(PCBDBLogin(CBInfo)^.szDbName));
      if Assigned(Database) and (PCBDBLogin(CBInfo)^.hDB = Database.Handle) then
        CloseDatabase(Database);
    end;
  except
    Result := cbrAbort;
...
begin
  for I := FDatabases.Count - 1 downto 0 do
    if TDatabase(FDatabases[I]).Temporary and (TDatabase(FDatabases[I]).FRefCount = 0) then TDatabase(FDatabases[I]).Free;
end;
...
begin
  ...
  try
    ...
    begin
      OemToCharA(ConfigDesc.szValue, ConfigDesc.szValue);
      List.Add(Format('%s=%s', [ConfigDesc.szNodeName, ConfigDesc.szValue]));
    end;
  finally
    DbiCloseCursor(Cursor);
  end;
...
begin
  ...
  try
    ...
    GetAliasNames(Names);
    for I := 0 to FDatabases.Count - 1 do
      if not IsDirectory(TDatabase(FDatabases[I]).DatabaseName) then Names.Add(TDatabase(FDatabases[I]).DatabaseName);
    List.Assign(Names);
  finally
    Names.Free;
  end;
  ...
  try
    ...
    try
      ...
      try
        while DbiGetNextRecord(Cursor, dbiNOLOCK, @Desc, nil) = 0 do
        begin
          if Extensions and (Desc.szExt[0] <> #0) then
            StrCat(StrCat(Desc.szName, '.'), Desc.szExt);
          NativeToAnsi(Database.Locale, Desc.szName, Name);
          List.Add(string(Name));
        end;
      finally
        DbiCloseCursor(Cursor);
...
begin
  ...
  begin
    ...
    begin
      ...
      TMonitor.Enter(FCSect);
      try
        FSessionNumber := Sessions.FSessionNumbers.OpenBit;
        Sessions.FSessionNumbers[FSessionNumber] := True;
      finally
        TMonitor.Exit(FCSect);
      end;
...
begin
  ...
  try
    Check(DbiCfgModifyRecord(nil, PAnsiChar(AnsiString(Format(Path, [Node]))), ParamList.FieldCount,
...
begin
  FTraceFlags := Value;
  for I := FDatabases.Count - 1 downto 0 do
    TDatabase(FDatabases[I]).TraceFlags := FTraceFlags;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      for I := FDatabases.Count - 1 downto 0 do
        if TDatabase(FDatabases[I]).Temporary then TDatabase(FDatabases[I]).Free else TDatabase(FDatabases[I]).Close;
      ...
      if (DbiGetProp(hDBIObj(Handle), curFIELDTYPENAME, @TypeDesc,
        ...
        begin
          if (ObjectField.DataType = ftArray) and SparseArrays and
            ...
            GetObjectTypeNames(TObjectField(ObjectField.Fields[0]).Fields)
          else
...
procedure TBDEDataSet.ClearCalcFields(Buffer: PByte);
begin
...
begin
  ...
  ClearBlobCache(Buffer);
  PRecInfo(Buffer + FRecInfoOfs)^.UpdateStatus := TUpdateStatus(usInserted);
  PRecInfo(Buffer + FRecInfoOfs)^.BookMarkFlag := bfInserted;
  PRecInfo(Buffer + FRecInfoOfs)^.RecordNumber := -1;
end;
...
begin
  ...
  case Status of
    DBIERR_NONE:
      begin
        PRecInfo(Buffer + FRecInfoOfs)^.UpdateStatus := TUpdateStatus(FRecProps.iRecStatus);
        PRecInfo(Buffer + FRecInfoOfs)^.BookmarkFlag := bfCurrent;
        case FRecNoStatus of
          rnParadox: PRecInfo(Buffer + FRecInfoOfs)^.RecordNumber := FRecProps.iSeqNum;
          rnDBase: PRecInfo(Buffer + FRecInfoOfs)^.RecordNumber := FRecProps.iPhyRecNum;
        else
          PRecInfo(Buffer + FRecInfoOfs)^.RecordNumber := -1;
        end;
        ClearBlobCache(Buffer);
...
begin
  ...
  if State = dsCalcFields then
    BufPtr := TRecordBuffer(CalcBuffer) else
...
function TBDEDataSet.GetActiveRecBuf(var RecBuf: PByte): Boolean;
begin
  case State of
    dsBlockRead: RecBuf := FBlockReadBuf + (FBlockBufOfs * FRecordSize);
    dsBrowse: if IsEmpty then RecBuf := nil else RecBuf := PByte(ActiveBuffer);
    dsEdit, dsInsert: RecBuf := PByte(ActiveBuffer);
    dsSetKey: RecBuf := PByte(FKeyBuffer) + SizeOf(TBDEKeyBuffer);
    dsCalcFields: RecBuf := PByte(CalcBuffer);
    dsFilter: RecBuf := @FFilterBuffer[0];
    dsNewValue: if FInUpdateCallback then
                  RecBuf := FUpdateCBBuf.pNewRecBuf else
                  RecBuf := PByte(ActiveBuffer);
    dsOldValue: if FInUpdateCallback then
                  RecBuf := FUpdateCBBuf.pOldRecBuf else
                  RecBuf := @GetOldRecord[0];
  else
    RecBuf := nil;
  end;
  Result := RecBuf <> nil;
...
var
  ...
  FieldName, LName: AnsiString;
  FieldDesc: BDEFLDDesc;
  LFieldDef: TFieldDef;
begin
  FieldDesc := FieldDescs[DescNo];
  Inc(DescNo);
  NativeToAnsi(Locale, FieldDesc.szName, FieldName);
  I := 0;
  LName := FieldName;
  while FieldDefs.IndexOf(string(LName)) >= 0 do
  begin
    Inc(I);
    LName := AnsiString(Format('%s_%d', [FieldName, I]));
  end;
  ...
  if RequiredFields.Size > FieldID then
    LRequired := RequiredFields[FieldID]
  else
    LRequired := False;
  case FieldDesc.iFldType of
    ...
    fldINT32:
      if FieldDesc.iSubType = fldstAUTOINC then
      begin
        LType := ftAutoInc;
        LRequired := False;
      end;
    fldFLOAT:
      if FieldDesc.iSubType = fldstMONEY then LType := ftCurrency;
    fldBCD:
      begin
    ...
    fldBLOB:
      begin
        LSize := FieldDesc.iUnits1;
        if (FieldDesc.iSubType >= fldstMEMO) and (FieldDesc.iSubType <= fldstBFILE) then
  ...
  if FieldDesc.efldrRights = fldrREADONLY then
    LFieldDef.Attributes := LFieldDef.Attributes + [faReadonly];
  if FieldDesc.iSubType = fldstFIXED then
    LFieldDef.Attributes := LFieldDef.Attributes + [faFixed];
  LFieldDef.InternalCalcField := FieldDesc.bCalcField;
  case LType of
    ftADT:
      begin
        if FieldDesc.iSubType = fldstADTNestedTable then
          LFieldDef.Attributes := LFieldDef.Attributes + [faUnNamed];
        for I := 0 to FieldDesc.iUnits1 - 1 do
          AddFieldDesc(FieldDescs, DescNo, FieldID, RequiredFields, LFieldDef.ChildDefs);
      end;
    ftArray:
      begin
        I := FieldID;
        StrCat(StrCopy(FieldDescs[DescNo].szName, FieldDesc.szName),'[0]');
        AddFieldDesc(FieldDescs, DescNo, I, RequiredFields, LFieldDef.ChildDefs);
...
begin
  ...
  else if not GetActiveRecBuf(RecBuf) then Exit;
...
function TBDEDataSet.GetFieldData(FieldNo: Integer; var Buffer: TValueBuffer): Boolean;
var
  IsBlank: LongBool;
  RecBuf: PByte;
  Status: DBIResult;
...
function TBDEDataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
var
  RecBuf: PByte;
begin
  ...
  begin
    ...
    begin
      if FConstraintLayer and Field.HasConstraints and (State in [dsEdit, dsInsert]) then
        Check(DbiVerifyField(FHandle, Field.FieldNo, Buffer, Blank));
      Check(DbiPutField(FHandle, Field.FieldNo, RecBuf, Buffer));
    end;
  end
  else {fkCalculated, fkLookup}
  begin
    Boolean(RecBuf[FRecordSize + Field.Offset]) := LongBool(Buffer);
    if Boolean(RecBuf[FRecordSize + Field.Offset]) then Move(Buffer[0], RecBuf[FRecordSize + Field.Offset + 1], Field.DataSize);
  end;
...
begin
  DbiFreeBlob(Handle, PByte(ActiveBuffer), Field.FieldNo);
end;
...
begin
  Check(DbiGetRecord(FHandle, dbiWriteLock, PByte(ActiveBuffer), nil));
  ClearBlobCache(PByte(ActiveBuffer));
end;
...
begin
  ...
  if State = dsEdit then
    Check(DbiModifyRecord(FHandle, PByte(ActiveBuffer), True)) else
    Check(DbiInsertRecord(FHandle, dbiNoLock, PByte(ActiveBuffer)));
end;
...
begin
  if (DataSetField <> nil) and (DataSetField.DataType = ftReference) then
    Check(DbiForceRecordReread(FHandle, PByte(ActiveBuffer))) else
...
begin
  Check(DbiSetToBookmark(FHandle, @Buffer[FBookmarkOfs]));
end;
...
procedure TBDEDataSet.GetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
...
procedure TBDEDataset.SetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
...
begin
  ...
  DbiInitRecord(FHandle, PByte(Buffer) + SizeOf(TBDEKeyBuffer));
  Result := Buffer;
end;
...
var
  ...
  StartKey, EndKey, IndexBuffer: PByte;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        StartKey := PByte(RangeStart) + SizeOf(TBDEKeyBuffer);
      ...
      begin
        EndKey := PByte(RangeEnd) + SizeOf(TBDEKeyBuffer);
...
procedure TBDEDataSet.SetLinkRanges(MasterFields: TList<TField>);
var
  I: Integer;
...
function TBDEDataSet.MapsToIndex(Fields: TList<TField>;
...
function TBDEDataSet.MapsToIndex(Fields: TList; CaseInsensitive: Boolean): Boolean;
var
  I: Integer;
  HasStr: Boolean;
begin
...
{$WARNINGS OFF}
function TBDEDataSet.CreateLookupFilter(Fields: TList<TField>; const Values: Variant;
...
function TBDEDataSet.ConstraintCallBack(Req: DsInfoReq;
  ...
  begin
    StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ImportedConstraint)));
    StrCopy(ADataSources.szSQLExprCustom, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].CustomConstraint)));
    StrCopy(ADataSources.szErrStrCustom, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ErrorMessage)));
    StrCopy(ADataSources.szErrStrImport, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ErrorMessage)));
  end;
...
procedure TBDEDataSet.DataEvent(Event: TDataEvent; Info: NativeInt);
  ...
  begin
    ParentPosition := 0;
    if not FParentDataSet.IsEmpty then
      for I := 0 to FParentDataSet.BookmarkSize - 1 do
        ParentPosition := ParentPosition + Byte(TRecordBuffer(FParentDataSet.ActiveBuffer)[FParentDataSet.FBookmarkOfs+I]);
...
begin
  ...
  begin
    if Prev <> nil then
      ...
      PrevErr := 0;
      Result := EUpdateError.Create(E.Message, '', EDBEngineError(E).Errors[0].ErrorCode, PrevErr, E);
  end
  else
...
begin
  ...
  if DataSetField.DataType = ftDataSet then
    Check(DbiOpenNestedTable(PHandle, DataSetField.FieldNo, DataSetField.ReadOnly, False, Result))
  else
    Check(DbiOpenRef(PHandle, DataSetField.FieldNo, DataSetField.ReadOnly, False, Result));
...
begin
  Result := Sessions.List[FSessionName].DoOpenDatabase(FDatabasename, Self.Owner);
end;
...
begin
  ...
  for I := 0 to Fields.Count - 1 do
    ...
    begin
      ...
      Check(DbiSetProp(hDbiObj(FHandle), PropFlags[Fields[I].AutoGenerateValue], LongInt(@ColDesc)));
    end;
end;
...
begin
  ...
  begin
    ...
    List.Add(Attr);
    Attr^.Name := 'LCID';
    Attr^.Value := Integer(TOSBLObj(Locale^).LdLCID);
...
function TDBDataSet.PSExecuteStatement(const ASQL: string; AParams: TParams): Integer;
var
  InProvider: Boolean;
begin
  InProvider := SetDBFlag(dbfProvider, True);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          GetMappingNames;
          LFieldDef := Source.FieldDefs.Find(SourceName);
          Destination.FieldDefs.Add(DestName, LFieldDef.DataType, LFieldDef.Size, LFieldDef.Required);
        end;
      end;
...
begin
  if FOwner.Active then FOwner.OpenIndexFile(S);
  FOwner.IndexDefs.Updated := False;
...
begin
  if FOwner.Active then
    for I := 0 to Count - 1 do FOwner.CloseIndexFile(Strings[I]);
  inherited Clear;
end;
...
begin
  if FOwner.Active then FOwner.CloseIndexFile(Strings[Index]);
  FOwner.IndexDefs.Updated := False;
...
var
  ...
  FieldName: string;
begin
  if IndexDesc.szTagName[0] = #0 then
  begin
    SName := IndexDesc.szName;
    Source := '';
  end
  else
  begin
    SSource := IndexDesc.szName;
    SName := IndexDesc.szTagName;
  ...
  IndexOptions := [];
  if IndexDesc.bPrimary then Include(IndexOptions, ixPrimary);
  if IndexDesc.bUnique then Include(IndexOptions, ixUnique);
  if IndexDesc.bDescending then Include(IndexOptions, ixDescending);
  if IndexDesc.bCaseInsensitive then Include(IndexOptions, ixCaseInsensitive);
  if not IndexDesc.bMaintained then Include(IndexOptions, ixNonMaintained);
  if IndexDesc.bExpIdx then
  begin
    NativeToAnsi(Locale, IndexDesc.szKeyExp, FieldExpression);
  ...
  begin
    FieldExpression := '';
    DescFields := '';
    for I := 0 to IndexDesc.iFldsInKey - 1 do
    begin
      FieldName := FieldDefList[IndexDesc.aiKeyFld[I] - 1].Name;
      ConcatField(FieldExpression, FieldName);
      if IndexDesc.abDescending[I] then
        ConcatField(DescFields, FieldName);
    end;
    if IndexDesc.bDescending and (DescFields = FieldExpression) then
      DescFields := '';
  end;
...
begin
  ...
  IndexDesc.bPrimary := ixPrimary in Options;
  IndexDesc.bUnique := ixUnique in Options;
  IndexDesc.bDescending := (ixDescending in Options) and (DescFields = '');
  IndexDesc.bMaintained := not (ixNonMaintained in Options);
  { -1 for True is ignored in the Paradox driver }
  Word(IndexDesc.bCaseInsensitive) := Word(ixCaseInsensitive in Options);
  if IsXBaseTable then
  begin
    if IndexDesc.bMaintained then
      AnsiToNative(Locale, Name, IndexDesc.szTagName, SizeOf(IndexDesc.szTagName) - 1)
    else
      AnsiToNative(Locale, Name, IndexDesc.szName, SizeOf(IndexDesc.szName) - 1);
  end
  else
    AnsiToNative(Locale, Name, IndexDesc.szName, SizeOf(IndexDesc.szName) - 1);
  if ixExpression in Options then
  begin
    IndexDesc.bExpIdx := True;
    AnsiToNative(Locale, FieldExpression, IndexDesc.szKeyExp, SizeOf(IndexDesc.szKeyExp) - 1);
  end
  else
  begin
    Pos := 1;
    while (Pos <= Length(FieldExpression)) and (IndexDesc.iFldsInKey < DBIMAXFLDSINKEY) do
    begin
      IndexDesc.aiKeyFld[IndexDesc.iFldsInKey] :=
        FieldDefs.Find(ExtractFieldName(FieldExpression, Pos)).FieldNo;
      IndexDesc.abDescending[IndexDesc.iFldsInKey] := IndexDesc.bDescending;
      Inc(IndexDesc.iFldsInKey);
    end;
    if (DescFields <> '') then
    begin
      IndexDesc.bDescending := True;
      Pos := 1;
      while Pos <= Length(DescFields) do
        IndexDesc.abDescending[IndexFieldOfs(ExtractFieldName(DescFields, Pos))] := True;
    end;
  end;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        I := IndexDefs.IndexOf(IndexStr);
        if I <> -1 then
          IndexStr := IndexDefs.Items[I].Source
        else
...
var
  ...
  begin
    while not CheckOpen(DbiOpenIndexList(DBHandle, NativeTableName,
      GetTableTypeName, LCursor)) do {Retry};
    try
      while DbiGetNextRecord(LCursor, dbiNoLock, @IndexDesc, nil) = 0 do
        if IndexDesc.bMaintained then
        begin
...
begin
  ...
  try
    RecBuffer := PByte(KeyBuffer) + SizeOf(TBDEKeyBuffer);
...
procedure TTable.GetDetailLinkFields(MasterFields, DetailFields: TList<TField>);
var
  i: Integer;
  Idx: TIndexDef;
begin
...
var
  LBatchMove: TBatchMove;
begin
  LBatchMove := TBatchMove.Create(nil);
  try
    LBatchMove.Destination := Self;
    LBatchMove.Source := ASource;
    LBatchMove.Mode := AMode;
    LBatchMove.Execute;
    Result := LBatchMove.MovedCount;
  finally
    LBatchMove.Free;
  end;
end;
...
var
  ...
  begin
    ...
    SetLength(TempFieldDescs, TableDesc.iFldCount);
    for I := 0 to FieldDefs.Count - 1 do
    begin
      EncodeFieldDesc(TempFieldDescs[I], FieldDefs[I].Name, FieldDefs[I].DataType, FieldDefs[I].Size, FieldDefs[I].Precision);
      if FieldDefs[I].Required then Inc(TableDesc.iValChkCount);
    end;
    SetLength(FieldDescs, TableDesc.iFldCount);
    TableDesc.pFldDesc := BDE.PFLDDesc(FieldDescs);
    PSQLLName := nil;
    if Database.IsSQLBased then
      if DbiGetLdNameFromDB(DBHandle, nil, SQLLName) = 0 then
        PSQLLName := SQLLName;
    Check(DbiTranslateRecordStructure(nil, TableDesc.iFldCount, BDE.PFLDDesc(TempFieldDescs),
      GetDriverTypeName(DriverTypeName), PSQLLName, TableDesc.pFLDDesc, False));
  end;
  ...
  begin
    ...
    for I := 0 to IndexDefs.Count - 1 do
      EncodeIndexDesc(IndexDescs[I], IndexDefs[I].Name, IndexDefs[I].FieldExpression, IndexDefs[I].Options, IndexDefs[I].DescFields);
  end;
  ...
  begin
    if TableDesc.iValChkCount > 0 then
    begin
      SetLength(ValChecks, TableDesc.iValChkCount);
      ValCheckNo := 0;
      for I := 0 to FieldDefs.Count - 1 do
      ...
      TableDesc.pvchkDesc := BDE.pVCHKDesc(ValChecks);
    end;
  end;
...
procedure TTable.EncodeFieldDesc(var FieldDesc: BDEFLDDesc;
  const Name: string; DataType: TFieldType; Size, Precision: Integer);
begin
  AnsiToNative(Locale, Name, FieldDesc.szName, SizeOf(FieldDesc.szName) - 1);
  FieldDesc.iFldType := FldTypeMap[DataType];
  FieldDesc.iSubType := FldSubTypeMap[DataType];
  case DataType of
...
begin
  ...
  try
    while DbiGetNextRecord(Cursor, dbiNOLOCK, @Desc, nil) = 0 do
    begin
      NativeToAnsi(DBLocale, Desc.szName, ParamName);
      if (TParamType(Desc.eParamType) = ptResult) and (ParamName = '') then
        ParamName := AnsiString(SResultName);
      if Desc.uFldType < MAXLOGFLDTYPES then ParamDataType := DataTypeMap[Desc.uFldType]
      else ParamDataType := ftUnknown;
      case Desc.uFldtype of
        fldFloat:
           if Desc.uSubType = fldstMONEY then ParamDataType := ftCurrency;
        fldBlob:
           if (Desc.uSubType >= fldstMEMO) and (Desc.uSubType <= fldstBFILE) then
             ParamDataType := BlobTypeMap[Desc.uSubType];
      end;
      LParam := TParam(FParams.Add);
      LParam.ParamType := TParamType(Desc.eParamType);
      LParam.DataType := ParamDataType;
      LParam.Name := string(ParamName);
    end;
    SetServerParams;
  finally
    DbiCloseCursor(Cursor);
...
begin
  ...
  for I := 0 to Params.Count - 1 do
  begin
    FServerDescs[I].ParamName := AnsiString(TParam(Params[I]).Name);
    FServerDescs[I].BindType := TParam(Params[I]).DataType;
  end;
end;
  ...
  begin
    ...
    begin
      for J := Low to Params.Count - 1 do
        if TParam(Params[J]).Name = string(FServerDescs[I].ParamName) then
          if (TParam(Params[J]).DataType <> FServerDescs[I].BindType) then
          begin
            Result := False;
            Exit;
          end
...
begin
  ...
  NumBytes := 0;
  for I := 0 to FParams.Count - 1 do
    if Params[I].DataType in [ftString, ftFixedChar] then Inc(NumBytes, 255 + 2)
  ...
  begin
    if Params[I].DataType = ftUnknown then
      DatabaseErrorFmt(SNoParameterValue, [Params[I].Name], Self);
    if Params[I].ParamType = ptUnknown then
      DatabaseErrorFmt(SNoParameterType, [Params[I].Name], Self);
    if FBindMode = pbByName then
      AnsiToNative(Locale, Params[I].Name, FParamDescs[I].szName, SizeOf(FParamDescs[I].szName) - 1)
    else FParamDescs[I].uParamNum := I + 1;
    FParamDescs[I].eParamType := STMTParamType(Params[I].ParamType);
    FParamDescs[I].uFldType := FldTypeMap[Params[I].DataType];
    FParamDescs[I].uSubType := FldSubTypeMap[Params[I].DataType];
    if FParamDescs[I].uFldType = fldZString then
    begin
      FParamDescs[I].uLen := 255;
      FParamDescs[I].iUnits1 := 255;
    end
    else if FParamDescs[I].uFldType = fldADT then
    begin
      FParamDescs[I].iUnits1 := Params[I].Value;
      FParamDescs[I].iUnits2 := Params[I].Value;
      AnsiToNative(Locale, Params[I].Name, FParamDescs[I].szName, SizeOf(FParamDescs[I].szName) - 1)
    end
    else
      FParamDescs[I].uLen := GetParamDataSize(Params[I]);
    FParamDescs[I].uOffset := Offset;
    Inc(Offset, FParamDescs[I].uLen);
    FParamDescs[I].uNullOffset := NumBytes - 2 * (I + 1);
  ...
  try
    ...
    begin
      ...
      begin
        GetParamData(Params[I], CurPtr, DrvLocale);
        IntPtr := @FRecordBuffer[NumBytes - 2 * (I + 1)];
        if Params[I].IsNull then IntPtr^ := IndNull
        else IntPtr^ := 0;
      end;
      if Params[I].DataType in [ftString, ftFixedChar] then
      begin
        Inc(CurPtr, 255);
        ...
        begin
          FParamDescs[I].uLen := GetParamDataSize(Params[I]);
          if FParamDescs[I].uFldType = fldZString then
            FParamDescs[I].iUnits1 := Params[I].GetDataSize - 1 {Do not include null terminator}
          else
            FParamDescs[I].iUnits1 := Params[I].GetDataSize;
        end
      end
      else
...
procedure TQuery.GetDetailLinkFields(MasterFields, DetailFields: TList<TField>);
...
begin
  ...
  if Query[UpdateKind].RowsAffected <> 1 then DatabaseError(SUpdateFailed);
end;
...
begin
  ...
  begin
    ...
    if Old then Param.AssignFieldValue(Field, Field.OldValue)
    else
    begin
...
begin
  ...
  begin
    if Mode = bmRead then
    begin
      FCached := FDataSet.FCacheBlobs and (FBuffer = PByte(FDataSet.ActiveBuffer)) and
      ...
      case Status of
        DBIERR_NONE, DBIERR_ENDOFBLOB:
          begin
            ...
            if FDataset.FCacheBlobs and (FBuffer = PByte(FDataSet.ActiveBuffer)) and
