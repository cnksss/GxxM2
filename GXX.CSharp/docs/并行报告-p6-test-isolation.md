...
| 覆盖 | **23 个静态全局类型、1107 个成员槽位**（1082 可赋值 + 25 只读数组内容位；M2Config 占 1048），反射枚举、**非手写清单** |
...
* **字段**：`BindingFlags.Public|NonPublic|Static|DeclaredOnly` 全收 —— 排除 `const`（不可写，13 个）、排除「非数组的 `static readonly`」（36 个，见 §8）、排除自动属性后备字段（由属性槽接管，避免同一存储快照两次）。
...
* **其它在飞车道的 src 侧静态接缝**：`NpcSeams`（测试里 200 处引用 / 13 处 Reset）、`AuctionDbRunSeam`（50/13）、`DbLayerRunSeam`（7/4）。它们是 **Npc\* / DbLayer\* 车道的独占区**，且其测试已经自己 Reset；本车道引用它们会把构建耦合到正在改动的文件上，故**不碰**，列出交给上级决定（若要，加进 `M2ConfigIsolationCoverage.Types` 一行即可生效）。
* **其它车道自己的测试侧接缝**（`CustomMagicFormGlobals` / `CustomMagicMessageBoxSeam` / `TVtEditorFactory` / `DbLayerTestKit` …）：定义在测试工程里、由各车道测试基类 `Reset`，不越界。
