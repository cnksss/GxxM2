...
> ## ⛔ 2026-09-21 更正（台账 §49，**引用本表任何"已移植"数字前必读**）
> 只读复核车道 `p12-e2only-review` 用方法级抽样证明：**`ObjMon.pas` 的 204 条实现里 0 条真正移植**（裁决 C）。
> 那 46 个 `ObjMon*Core.cs` 合计约 6,300 个方法，**其中约 1,917 条是裸 `=> true;` 恒真断言**
> （调度方独立复核：`tools/audit-stubs.ps1` 实测 **1,917**）；`Engine/ObjBase.cs:216` 的 `TMonster`
> 是 18 行近似物，而原文 `TMonster.Run`（`ObjMon.pas:1121-1391`）是 270+ 行。
> 因此**本表的"已移植"判定（含 §3 的 37/55）不可再作为完成度依据** ——
> 本表 §5/§6 自己已声明判据有假阴性、§8–§9 记录四/五种判据全部失败，现再补一条**更根本**的：
> **"类被声明"既不等于"方法被实现"，也不等于"实现不是恒真桩"。**
> **正确口径**：`ObjMon` = **REFUTED（真缺口）**，已开车道 `par/p13-m2-objmon-real` 就地修复。
> 本文件保留作**历史记录**，只可用于"选题索引"。
