...
$PARTIAL_UNITS = @(
    ...
    'FState'           # 100/533 = 18.76% (round 3); remaining throw shells 436 (from 515)
    ...
    # zero error).  Those 220 are NOT gaps -- they are the original's subclass hooks, and keeping the
    # throw shell is CORRECT (1:1).  So FState's real remaining work is 208 concrete members, not 432.
    # Do not let a future lane "implement" the 220 hooks: that would invent behaviour the original
    # never had (a third implementation, ledger 14.2).
)
