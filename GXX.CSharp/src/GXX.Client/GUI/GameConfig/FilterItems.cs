...
public class TFileItemDB
{
    ...
    // ================================================================================
    // GetValidStr3 —— **转调 GXX.Core.Util.HUtil32**（并行批次 P2-core-rtl 去重）
    // --------------------------------------------------------------------------------
    // 本方法原先是对 Source/Common/HUtil32.pas:1243-1341 的逐字复刻，因为当时
    // GXX.Core.Util.HUtil32.GetValidStr3 与原文不符（以分隔符开头时 Dest 返回空串，
    // 会让 LoadFormList 取不到 sItemName 而整行被丢弃）。
    // 该缺陷已在 GXX.Core 侧按 {$ELSE} ANSI 分支根因修好
    // （GXX.Core/Util/HUtil32.cs:241-301，并行派发台账 §9.4），故此处改为**转调**：
    // 同一算法只保留一份实现，避免与 M2/Client/DBServer 各处的解析行为漂移。
    // 保留本包装是为了不动 FilterItems.pas:186-193 调用点的形状（原文即 7 次连续调用）。
    // 原文语义：丢掉开头连续的任意个分隔符；Dest = 随后第一段非分隔符文本；
    // 返回值 = 该段之后第一个分隔符**之后**的剩余串（该分隔符被吃掉）；全串皆分隔符时 Dest = 原串。
    // ================================================================================

    /// <summary>
    /// Delphi <c>HUtil32.GetValidStr3(Str:string; var Dest:string; const Divider:array of Char):string</c>
    /// （HUtil32.pas:1243-1341）—— 转调 <see cref="GXX.Core.Util.HUtil32.GetValidStr3"/>。
    /// </summary>
    public static string GetValidStr3(string Str, ref string Dest, char[] Divider)
        => HUtil32.GetValidStr3(Str, ref Dest, Divider);
}
